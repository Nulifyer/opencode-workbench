...
| Extension Host E2E    | Activate the extension in VS Code 1.106.0, verify commands and the editor webview, create an empty session, and prove startup sends no prompt. | `xvfb-run -a deno task test:vscode:e2e`                                                                                                  |
| Harness UX            | Run the nineteen provider-free lifecycle, queue, permission, recovery, isolation, run, diff, continuity, GitHub, and accessibility scenarios.  | `deno task test:harness:ux`                                                                                                              |
...
The pull-request compatibility workflow runs the Extension Host E2E gate in a
separate Xvfb job. Set `VSCODE_E2E_VERSION` to exercise another VS Code release,
or `VSCODE_E2E_EXECUTABLE` to use an already installed executable. The E2E uses
an isolated trusted workspace and authenticated loopback fixture; it does not
send a model request or inspect webview DOM internals.
