...
export function parseBridgeRegistry(value: unknown): BridgeRegistry {
  ...
  const entries = value.entries.map((entry): BridgeEntry => {
    if (!isRecord(entry) || typeof entry.id !== "string" || entry.id.length === 0 || entry.id.length > 128 ||
      ...
      !Number.isSafeInteger(entry.pid) || Number(entry.pid) <= 0 || !Number.isSafeInteger(entry.updatedAt) || Number(entry.updatedAt) < 0 ||
      !Array.isArray(entry.operations) || entry.operations.length === 0 || entry.operations.length > Object.keys(BRIDGE_OPERATIONS).length ||
      !entry.operations.every((operation) => typeof operation === "string" && Object.hasOwn(BRIDGE_OPERATIONS, operation)) ||
...
export function bridgeEntryIsFresh(entry: BridgeEntry, now: number, isAlive: (pid: number) => boolean): boolean {
  return now - entry.updatedAt <= BRIDGE_STALE_AFTER_MS && entry.updatedAt <= now + BRIDGE_STALE_AFTER_MS && isAlive(entry.pid)
}
