import { type AtomicAdapter, AtomicJsonStore } from "../src/atomic-store.ts"
...
Deno.test("atomic store serializes mutations and renames temporary state", async () => {
  ...
  equal(adapter.operations, [
    "write:state.json.tmp-1",
    "rename:state.json.tmp-1:state.json",
    "write:state.json.tmp-2",
    "rename:state.json.tmp-2:state.json",
  ])
})
