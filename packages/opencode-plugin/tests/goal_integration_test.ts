...
function transcriptMessage(request: Record<string, unknown>, sessionID: string): Record<string, unknown> {
  ...
  if (!body?.messageID || !body.parts?.length) {
    throw new Error("Continuation request did not carry stable message and part IDs")
  }
  return {
...
Deno.test("goal configure tool applies one atomic form with generation and Plan safety", async () => {
  ...
  try {
    ...
    hooks = await PluginModule.server(
      { client, directory: "/workspace", worktree: "/workspace" } as never,
    ) as unknown as GoalHooks

    const sessionID = "ses_atomic_configure"
    const created = parsed(
      await hooks.tool.create_goal.execute({ objective: "Original objective" }, context(sessionID)),
    ).goal as { settlementGeneration?: number }
    ...
    if (
      configured.objective !== "Updated objective" || configured.acceptanceCriteria?.length !== 2 ||
      configured.tokenBudget !== 2_000
    ) {
    ...
    if (
      !configured.verifier?.enabled || configured.verifier.model !== "provider/model" ||
      configured.verifier.agent !== "verify" || configured.verifier.repeatedBlockThreshold !== 2
    ) {
    ...
    if (await Deno.readTextFile(statePath) !== beforeStale) {
      throw new Error("A stale configure tool call mutated persisted goal state")
    }
    ...
    ).goal as { objective?: string; status?: string; acceptanceCriteria?: string[]; verifier?: { enabled?: boolean } }
    if (
      legacy.objective !== "Keep the legacy objective" || legacy.status !== "active" ||
      legacy.acceptanceCriteria?.[0] !== "Legacy fields remain accepted" || !legacy.verifier?.enabled
    ) {
...
Deno.test("native goal hooks integrate lifecycle, persistence, policy, compaction, and cleanup", async () => {
  ...
  try {
    ...
    const client = {
      session: {
        ...
        promptAsync: async (request: Record<string, unknown>) => {
          ...
          return admissionError === undefined
            ? { data: undefined, error: undefined }
            : { data: undefined, error: admissionError }
        },
      },
    }
    ...
    if (!commands?.goal || !commands["goal-unlimited"]) {
      throw new Error("Goal commands were not registered through the plugin config hook")
    }
    ...
    const created = parsed(
      await hooks.tool.create_goal.execute({
        objective: "Implement and verify native goal integration",
        max_auto_turns: 2,
      }, context(sessionID)),
    )
    if ((created.goal as { status?: string })?.status !== "active") {
      throw new Error("Goal tool did not create an active goal")
    }

    await hooks.tool.update_goal_checkpoint.execute(
      { summary: "Native goal tools loaded and persisted state." },
      context(sessionID),
    )
    const current = parsed(await hooks.tool.get_goal.execute({}, context(sessionID))).goal as {
      lastCheckpoint?: { summary?: string }
    }
    if (current.lastCheckpoint?.summary !== "Native goal tools loaded and persisted state.") {
      throw new Error("Goal checkpoint was not persisted")
    }
    ...
    const firstRequest = prompts[0] as {
      path?: { id?: string }
      query?: { directory?: string }
      body?: { parts?: Array<{ type?: string; text?: string; synthetic?: boolean; metadata?: unknown }> }
    }
    if (
      firstRequest.path?.id !== sessionID || firstRequest.query?.directory !== "/workspace" ||
      firstRequest.body?.parts?.length !== 1 ||
      firstRequest.body.parts[0]?.type !== "text" || firstRequest.body.parts[0]?.text !== GOAL_CONTINUATION_PROMPT ||
      firstRequest.body.parts[0]?.synthetic !== true ||
    ...
    if (Number(prompts.length) !== 1) {
      throw new Error("A repeated idle event admitted another prompt before the prior prompt started")
    }
    ...
    if (promptCountAfterSecond !== 2 || afterSecond.autoTurns !== 2) {
      throw new Error("A completed goal turn did not admit and persist exactly one follow-up prompt")
    }

    await hooks.event({ event: { type: "session.status", properties: { sessionID, status: { type: "busy" } } } })
    await hooks.event({ event: { type: "session.idle", properties: { sessionID } } })
    await eventually(async () =>
      (parsed(await hooks.tool.get_goal.execute({}, context(sessionID))).goal as { status?: string }).status ===
        "usageLimited"
    )
    ...
    if (promptCountAtLimit !== 2 || limited.status !== "usageLimited") {
      throw new Error("The persisted auto-turn limit did not stop further prompts")
    }

    const compaction = { context: [] as string[] }
    await hooks["experimental.session.compacting"]({ sessionID }, compaction)
    if (!compaction.context[0]?.includes("Implement and verify native goal integration")) {
      throw new Error("Goal context was not preserved for compaction")
    }

    const restarted = await PluginModule.server(
      { client, directory: "/workspace", worktree: "/workspace" } as never,
    ) as unknown as GoalHooks
    activePlugins.push(restarted)
    const restored = parsed(await restarted.tool.get_goal.execute({}, context(sessionID))).goal as {
      objective?: string
    }
    if (restored.objective !== "Implement and verify native goal integration") {
      throw new Error("Goal did not survive plugin recreation")
    }

    const completed = parsed(
      await restarted.tool.update_goal.execute({
        status: "complete",
        evidence: "Integration lifecycle assertions passed.",
      }, context(sessionID)),
    )
    if ((completed.goal as { status?: string })?.status !== "complete") {
      throw new Error("Goal did not close with evidence")
    }
    ...
    await restarted.event({ event: { type: "session.idle", properties: { sessionID: controlSessionID } } })
    await resumedPrompt
    const controlled = parsed(await restarted.tool.get_goal.execute({}, context(controlSessionID))).goal as {
      objective?: string
      autoTurns?: number
    }
    ...
    await restarted.event({
      event: { type: "session.status", properties: { sessionID: controlSessionID, status: { type: "busy" } } },
    })
    await restarted.event({ event: { type: "session.idle", properties: { sessionID: controlSessionID } } })
    ...
    await failedPrompt
    await eventually(async () =>
      (parsed(await restarted.tool.get_goal.execute({}, context(failingSessionID))).goal as { status?: string })
        .status === "paused"
    )
    await restarted.dispose()
    const failed = parsed(await restarted.tool.get_goal.execute({}, context(failingSessionID))).goal as {
      status?: string
      lastStatus?: string
    }
    if (failed.status !== "paused" || !failed.lastStatus?.includes("prompt admission rejected")) {
      throw new Error("Prompt admission failure did not pause the goal with evidence")
    }

    await restarted.event({ event: { type: "session.deleted", properties: { info: { id: sessionID } } } })
    if (parsed(await restarted.tool.get_goal.execute({}, context(sessionID))).goal !== null) {
      throw new Error("Deleted session retained native goal state")
    }
...
Deno.test("goal continuation follows OpenCode status ordering and pauses on background failure", async () => {
  ...
  try {
    ...
    const client = {
      session: {
        status: async () =>
          statusError === undefined ? { data: {}, error: undefined } : { data: undefined, error: statusError },
        messages: async () => ({
          data: prompts.map((request) => transcriptMessage(request, sessionID)),
          error: undefined,
        }),
        promptAsync: async (request: Record<string, unknown>) => {
          prompts.push(request)
    ...
    await hooks.tool.get_goal.execute({}, context(sessionID))
    if (Number(prompts.length) !== 2) {
      throw new Error("A busy-to-idle transition did not admit exactly one continuation")
    }
    ...
    await hooks.event({
      event: {
        type: "session.error",
        properties: { sessionID, error: { message: "background prompt execution failed" } },
      },
    })
    const failed = parsed(await hooks.tool.get_goal.execute({}, context(sessionID))).goal as {
      status?: string
      lastStatus?: string
    }
    ...
    if (Number(prompts.length) !== 2) {
      throw new Error("Idle-before-error ordering admitted another continuation before failure handling")
    }

    const statusFailureSessionID = "ses_status_failure"
    statusError = { message: "status endpoint unavailable" }
    await hooks.tool.create_goal.execute(
      { objective: "Pause when idle cannot be verified" },
      context(statusFailureSessionID),
    )
    await hooks.event({
      event: { type: "session.status", properties: { sessionID: statusFailureSessionID, status: { type: "idle" } } },
    })
    await eventually(async () =>
      (parsed(await hooks!.tool.get_goal.execute({}, context(statusFailureSessionID))).goal as { status?: string })
        .status === "paused"
    )
    const statusFailure = parsed(await hooks.tool.get_goal.execute({}, context(statusFailureSessionID))).goal as {
      lastStatus?: string
    }
    if (!statusFailure.lastStatus?.includes("status endpoint unavailable")) {
      throw new Error("Status lookup failure did not pause the goal with a diagnostic")
    }
  } finally {
    secondPromptReturn.resolve()
    if (hooks) await hooks.dispose()
...
Deno.test("restart reconciles an admitted continuation by its durable transcript IDs", async () => {
  ...
  try {
    ...
    const first = await PluginModule.server(
      { client, directory: "/workspace", worktree: "/workspace" } as never,
    ) as unknown as GoalHooks
    plugins.push(first)
    const sessionID = "ses_crash_admitted"
    await first.tool.create_goal.execute(
      { objective: "Recover one admitted continuation", max_auto_turns: 3 },
      context(sessionID),
    )
    ...
    if (
      beforeRestart.autoTurns !== 1 || beforeRestart.pendingContinuation !== true ||
      !beforeRestart.pendingContinuationMessageID || !beforeRestart.pendingContinuationID
    ) {
...
Deno.test("restart retries an unobserved reservation with the same IDs and count", async () => {
  ...
  try {
    ...
    await first.tool.create_goal.execute(
      { objective: "Retry a reservation without double counting" },
      context(sessionID),
    )
    await first.event({ event: { type: "session.idle", properties: { sessionID } } })
    await eventually(async () => prompts.length === 1)
    ...
    if (
      recovered.autoTurns !== 1 || recovered.pendingContinuation !== true ||
      firstBody.messageID !== secondBody.messageID || firstBody.parts?.[0]?.id !== secondBody.parts?.[0]?.id
    ) {
  ...
  try {
    ...
    const client = {
      session: {
        status: async () => ({ data: {}, error: undefined }),
        messages: async () =>
          historyError === undefined ? { data: [], error: undefined } : { data: undefined, error: historyError },
        promptAsync: async () => {
          prompts += 1
          return { data: undefined, error: undefined }
        },
    ...
    if (
      recovered.status !== "paused" || recovered.autoTurns !== 1 || recovered.pendingContinuation !== false ||
      !recovered.lastStatus?.includes("explicitly resume") || prompts !== 1
    ) {
...
Deno.test("lost prompt response reconciles the admitted stable message from history", async () => {
  ...
  try {
    ...
    await hooks.tool.create_goal.execute(
      { objective: "Reconcile an admitted prompt after a lost response" },
      context(sessionID),
    )
    await hooks.event({ event: { type: "session.idle", properties: { sessionID } } })
    await eventually(async () => {
      const goal = parsed(await hooks!.tool.get_goal.execute({}, context(sessionID))).goal as {
        pendingContinuation?: boolean
      }
      return prompts === 1 && goal.pendingContinuation === false
    })
    const recovered = parsed(await hooks.tool.get_goal.execute({}, context(sessionID))).goal as {
      status?: string
      autoTurns?: number
    }
    if (recovered.status !== "active" || recovered.autoTurns !== 1) {
      throw new Error("Lost response was treated as a failed admission despite matching durable history")
    }
  } finally {
    if (hooks) await hooks.dispose()
    if (previous === undefined) Deno.env.delete("XDG_DATA_HOME")
