...
Deno.test("event evidence is bounded and excludes tool error content", () => {
  ...
  for (let index = 0; index < 201; index++) {
    appendEvidence(state, {
      id: String(index), scope: project, kind: "session_status", subject: "busy",
      sessionID: String(index), createdAt: index,
    })
  }
  ...
  const evidence = evidenceFromEvent({
    ...
    properties: {
      part: {
        type: "tool", tool: "read", sessionID: "s", messageID: "m", callID: "c",
...
Deno.test("skill approval only stages candidate metadata", () => {
  ...
  const evidence: Evidence = {
    id: "evidence", scope: project, kind: "skill_load", subject: "test-strategy",
    sessionID: "session", createdAt: 1,
  }
  appendEvidence(state, evidence)
  proposeCandidate(state, {
    scope: project, name: "focused-testing", rationale: "Capture repeated focused test usage.",
    evidenceIDs: [evidence.id], provenance,
  }, 2, "candidate")
