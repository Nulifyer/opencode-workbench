...
export function settlementReasons(
  ...
  if (
    Object.values(state.items).some((item) => item.state === "active" || item.state === "waiting")
...
export function lifecycleReducer(
  ...
  if (action.type === "promptQueued") {
    ...
    next = {
      ...
      activeTurnID: state.activeTurnID === action.turnID ? undefined : state.activeTurnID,
    }
    ...
    next = {
      ...
      activeTurnID: state.activeTurnID === turn.id ? undefined : state.activeTurnID,
      prompts: prompt
        ? updatePrompt(
          state.prompts,
          prompt.id,
