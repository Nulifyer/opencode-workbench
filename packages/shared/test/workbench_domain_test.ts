...
Deno.test("context receipts preserve metadata but cannot carry payload fields", () => {
  ...
  assertThrows(
    () => sanitizeContextReceipt({ ...receipt, items: [receipt.items[0]!, receipt.items[0]!] }),
    Error,
    "Duplicate",
  )
  assertThrows(
    () =>
      sanitizeContextReceipt({
        ...receipt,
        items: [{ ...receipt.items[0]!, range: { startLine: 3, startColumn: 1, endLine: 2, endColumn: 1 } }],
      }),
    Error,
    "range",
  )
  assertThrows(
    () =>
      sanitizeContextReceipt({ ...receipt, items: [{ ...receipt.items[0]!, uri: `file:///${"x".repeat(4_096)}` }] }),
    Error,
    "URI",
  )
})
    ...
    items: [
      {
        id: "url",
        kind: "url",
        label: "Authorization: Bearer top-secret",
        uri: "https://example.test/callback?access_token=top-secret#secret",
        revision: "token=revision-secret",
      },
...
Deno.test("walkthrough anchors must resolve against the exact complete diff", () => {
  const snapshot: DiffSnapshot = {
    ...
    files: [{
      ...
      additions: 2,
      deletions: 1,
      hunks: [{ header: "@@ -1 +1,2 @@", oldRange: { start: 1, end: 1 }, newRange: { start: 1, end: 2 } }],
    }],
  }
  validateWalkthrough({
    id: "walk",
    ...
    stops: [{
      ...
      anchors: [{ file: "src/main.ts", side: "modified", startLine: 1, endLine: 2, hunkHeader: "@@ -1 +1,2 @@" }],
    }],
  }, snapshot)
  assertThrows(
    () =>
  ...
  assertThrows(
    () =>
      validateWalkthrough({
        ...
        stops: [{
          id: "stop",
          title: "Invented range",
          explanation: "Outside the hunk",
          importance: "normal",
          anchors: [{
            file: "src/main.ts",
            side: "modified",
    ...
    () =>
      validateWalkthrough({
        ...
        stops: [{
          id: "stop",
          title: "Missing hunk",
          explanation: "No exact identity",
          importance: "normal",
          anchors: [{ file: "src/main.ts", side: "modified", startLine: 1, endLine: 1 }],
        }],
      }, snapshot),
    Error,
    "exact hunk",
...
Deno.test("walkthrough coverage cannot omit changed files", () => {
  ...
  const stop = {
    ...
    anchors: [{ file: "a.ts", side: "modified" as const, startLine: 1, endLine: 1, hunkHeader: "@@ -1 +1 @@" }],
  }
  assertThrows(
    () =>
      validateWalkthrough({
...
Deno.test("plan task artifacts retain only OpenCode provenance and document metadata", () => {
  ...
  assertThrows(
    () =>
      normalizeTaskArtifact({
        ...planArtifact(),
        payload: { ...planArtifact().payload, objective: "must never persist" },
      }),
    Error,
    "unsupported field objective",
  )
  assertThrows(
    () =>
      normalizeTaskArtifact({
        ...planArtifact(),
        payload: { phase: "ready", uri: "untitled:Plan", revision: "not-a-hash" },
      }),
    Error,
    "SHA-256",
  )
  assertThrows(
    () => normalizeTaskArtifact({ ...planArtifact(), producer: { sessionID: "other-session" } }),
    Error,
    "owning OpenCode session",
  )
})
...
Deno.test("handed-off plans require internally consistent OpenCode session metadata", () => {
  ...
  assertThrows(
    ...
    Error,
    "missing from relatedSessionIDs",
  )
})
...
Deno.test("review artifacts persist structured findings without raw diffs and enforce their hash", () => {
    ...
    payload: {
      ...
      document: {
        ...
        findings: [{
          id: "finding",
          title: "Missing guard",
          detail: "Validate the input first.",
          category: "correctness",
          severity: "high",
          anchors: [{ file: "src/main.ts", side: "modified", startLine: 4, endLine: 5, hunkHeader: "@@ -3,2 +3,4 @@" }],
        }],
      },
  ...
  assertThrows(
    () => normalizeTaskArtifact({ ...review, payload: { ...review.payload, rawDiff: "+secret" } }),
    Error,
    "unsupported field rawDiff",
  )
  assertThrows(
    () => normalizeTaskArtifact({ ...review, payload: { ...review.payload, diffHash: HASH_B } }),
    Error,
    "does not match",
  )
  assertThrows(
    () =>
      normalizeTaskArtifact({
        ...review,
        payload: { ...review.payload, dispositions: [{ findingID: "unknown", state: "open", updatedAt: 20 }] },
      }),
    Error,
    "unknown finding",
  )
...
Deno.test("goal verification artifacts contain bounded OpenCode attempts but no prompt material", () => {
  ...
  assertThrows(
    () => normalizeTaskArtifact({ ...verification, payload: { ...verification.payload, prompt: "private prompt" } }),
    Error,
    "unsupported field prompt",
  )
  assertThrows(
    () => normalizeTaskArtifact({ ...verification, producer: { sessionID: "unrelated-verifier" } }),
    Error,
    "one of its OpenCode attempts",
  )
  assertThrows(
    () => normalizeTaskArtifact({ ...verification, payload: { ...verification.payload, applied: false } }),
    Error,
    "cannot have appliedAt",
  )
})
...
Deno.test("run-comparison and context-capture artifacts remain deterministic metadata", () => {
  ...
  assertThrows(
    () => normalizeTaskArtifact({ ...comparison, producer: { sessionID: "fake-producer" } }),
    Error,
    "cannot claim",
  )
  ...
  const capture: TaskArtifact = {
    ...
    payload: {
      ...
      sources: [{
        ...
        kind: "selection",
        label: "src/main.ts:1",
        uri: "file:///repo/src/main.ts?token=private#fragment",
        contentHash: HASH_A,
        estimatedTokens: 3,
      }],
    },
  ...
  assertThrows(
    () => normalizeTaskArtifact({ ...capture, payload: { ...capture.payload, clipboard: "private bytes" } }),
    Error,
    "unsupported field clipboard",
  )
  assertThrows(
    () => normalizeTaskArtifact({ ...capture, payload: { ...capture.payload, receiptID: "other" } }),
    Error,
    "does not match",
  )
  assertThrows(
    () =>
      normalizeTaskArtifact({
        ...
        payload: {
          ...capture.payload,
          sources: Array.from({ length: 101 }, (_, index) => ({ id: `source-${index}`, kind: "file", label: "file" })),
        },
      }),
    Error,
    "at most 100",
