...
import { type ContextReceiptItem, sanitizeDurableMetadataUri } from "@opencode-workbench/shared"
...
export function assertBrowserScreenshotFileBeforeRead(input: {
  ...
  const expectedMime = extension === "png"
    ...
    : undefined
  if (
    !input.isFile || !Number.isSafeInteger(input.size) || input.size < 1 ||
    input.size > BROWSER_SCREENSHOT_BYTE_LIMIT ||
...
function textItem(
  id: string,
  kind: ContextReceiptItem["kind"],
  label: string,
  text: string,
  values: Partial<ContextReceiptItem> = {},
): ContextReceiptItem {
...
function approvedUrl(value: string): string {
  ...
  try {
    ...
    throw new Error("Approved URL must be a valid HTTP or HTTPS URL")
  }
  if (!["http:", "https:"].includes(parsed.protocol) || parsed.username || parsed.password) {
    throw new Error("Approved URL must be an HTTP or HTTPS URL without embedded credentials")
  }
...
export function captureBrowserContext(input: BrowserContextInput): BrowserContextCapture {
  ...
  if (input.editorSelection) {
    ...
    const range = {
      startLine: input.editorSelection.startLine,
      startColumn: input.editorSelection.startColumn,
      endLine: input.editorSelection.endLine,
      endColumn: input.editorSelection.endColumn,
    }
    if (
      ![range.startLine, range.startColumn, range.endLine, range.endColumn].every((value) =>
    ...
    receiptItems.push(
      textItem("browser:selection", "selection", "Explicit editor selection", selection, {
        uri,
        range,
        revision: input.editorSelection.revision,
      }),
    )
  }
  if (input.clipboardText) {
    const clipboard = bounded(input.clipboardText.text, 100_000, "Clipboard browser context")
    const label = input.clipboardText.kind === "console"
      ? "console output"
      : input.clipboardText.kind === "element"
      ? "inspected element metadata"
      : "terminal or task excerpt"
    sections.push(`## User-designated ${label}\n\n${clipboard}`)
    receiptItems.push(
      textItem(
        `browser:${input.clipboardText.kind}`,
        input.clipboardText.kind === "terminal-task" ? "terminal" : "debug",
  ...
  if (sections.length > 1) {
    files.push({
      ...
      mime: "text/markdown",
      url: `data:text/markdown;base64,${Buffer.from(sections.join("\n\n")).toString("base64")}`,
    })
  }
  if (input.screenshot) {
    assertBrowserScreenshotFileBeforeRead({
      ...
      size: input.screenshot.bytes.byteLength,
      isFile: true,
    })
    files.push({
      type: "file",
      ...
      mime: input.screenshot.mime,
      url: `data:${input.screenshot.mime};base64,${Buffer.from(input.screenshot.bytes).toString("base64")}`,
    })
    receiptItems.push({
      id: "browser:screenshot",
      kind: "attachment",
      label: input.screenshot.name,
      bytes: input.screenshot.bytes.byteLength,
      contentHash: contentHash(input.screenshot.bytes),
    })
  }
