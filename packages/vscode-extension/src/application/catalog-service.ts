...
export class CatalogService {
  ...
  constructor(preferences?: CatalogPreferences) {
    if (typeof preferences?.currentAgent === "string" && preferences.currentAgent.length <= 1_024) {
      this.currentAgent = preferences.currentAgent
    }
    if (typeof preferences?.lastModel === "string" && preferences.lastModel.length <= 1_024) {
      this.lastModel = preferences.lastModel
    }
    if (!this.lastModel) {
      const legacy = preferences?.agentModels?.find(([agent]) => agent === this.currentAgent) ??
        preferences?.agentModels?.at(-1)
      if (legacy && legacy.every((value) => typeof value === "string" && value.length <= 1_024)) {
        this.lastModel = legacy[1]
      }
    }
    for (const entry of preferences?.modelVariants?.slice(0, 500) ?? []) {
      if (
        !Array.isArray(entry) || entry.length !== 2 || typeof entry[0] !== "string" || entry[0].length > 1_024 ||
        (entry[1] !== null && (typeof entry[1] !== "string" || entry[1].length > 1_024))
      ) continue
      this.modelVariants.set(entry[0], entry[1] ?? undefined)
    }
  }

  validAgent(name?: string): name is string {
    return Boolean(name && this.agents.some((agent) => agent.name === name))
  }
  validModel(value?: string): value is string {
  ...
  remember(agent?: string, model?: string, variant?: string, rememberVariant = false): boolean {
    ...
    if (
      (rememberVariant || variant !== undefined) &&
      (!this.modelVariants.has(model) || this.modelVariants.get(model) !== (variant || undefined))
    ) {
  ...
  validate(agent?: string, model?: string, variant?: string, sessionAgent?: string): void {
    ...
    const effectiveModel = model === ""
      ? this.modelForAgent(effectiveAgent)
      : model || this.modelForAgent(effectiveAgent)
  ...
  apply(catalog: RuntimeCatalog, commands: CommandOption[], error?: string): void {
    ...
    this.mentionAgents = catalog.mentionAgents ?? []
    this.providers = catalog.providers ?? []
    this.models = catalog.models
    this.resources = catalog.resources ?? []
    this.status = error
      ? {
        status: this.models.length ? "stale" : "error",
        updatedAt: this.status.updatedAt,
        error: error.slice(0, 20_000),
      }
      : { status: "ready", updatedAt: Date.now() }
    this.defaultAgent = catalog.defaults?.agent
    this.defaultModel = catalog.defaults?.model
    this.defaultVariant = catalog.defaults?.variant
