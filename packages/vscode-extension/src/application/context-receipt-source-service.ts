...
import { type ContextReceipt, type ContextReceiptItem, sanitizeDurableMetadataUri } from "@opencode-workbench/shared"
...
function receiptItem(
  ...
  if (matches.length !== 1) {
    throw new Error(
      matches.length ? "Context receipt item ID is ambiguous" : "Context receipt item was not found",
    )
  }
  return matches[0]!
}
