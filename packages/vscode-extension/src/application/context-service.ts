import { type ContextReceipt, type ContextReceiptItem, sanitizeContextReceipt } from "@opencode-workbench/shared"
...
export class ContextReceiptService {
  ...
  forSession(sessionID: string): ContextReceipt[] {
    return [...this.receipts.values()].filter((receipt) => receipt.sessionID === sessionID).sort((left, right) =>
