...
export function captureDiagnostics(repository: string, observations: readonly DiagnosticObservation[]): DiagnosticsSnapshot {
  ...
  for (const observation of observations) {
    if (!path.isAbsolute(observation.file) || !nonNegativeInteger(observation.errors) || !nonNegativeInteger(observation.warnings)) continue
...
export function diagnosticsDelta(before: DiagnosticsSnapshot, after: DiagnosticsSnapshot): { status: EvidenceReference["status"]; summary: string } {
  ...
  return {
    status,
    summary: `Before: ${diagnosticsSummary(before)}; after: ${diagnosticsSummary(after)}; delta: ${signed(errorDelta)} errors, ${signed(warningDelta)} warnings, ${signed(fileDelta)} files`,
  }
}
