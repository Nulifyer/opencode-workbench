...
export function buildFusionBundle(group: RunGroup, mode: FusionMode, artifacts: FusionRunArtifact[]): FusionBundle {
  ...
  if (
    artifacts.length < 2 || artifacts.length > 5 ||
    new Set(artifacts.map((artifact) => artifact.runID)).size !== artifacts.length
  ) throw new Error("Fusion requires two to five unique source runs")
  let bytes = 0
  for (const artifact of artifacts) {
    if (
      !group.runs.some((run) =>
        run.id === artifact.runID && run.session.sessionID === artifact.sessionID &&
        run.session.directory === artifact.directory
      )
    ) throw new Error("Fusion artifact does not match its persisted source run")
    const expectedDiffHash = `sha256:${createHash("sha256").update(artifact.unifiedDiff).digest("hex")}`
    if (
      !artifact.diffSnapshot.complete || artifact.diffSnapshot.repository !== artifact.directory ||
      artifact.diffSnapshot.unifiedDiffHash !== expectedDiffHash
    ) throw new Error(`Fusion source ${artifact.runID} has an invalid exact diff manifest`)
    if (artifact.objectiveSummary.runID !== artifact.runID || artifact.evidence.length > 200) {
      throw new Error(`Fusion source ${artifact.runID} has invalid deterministic evidence`)
    }
    const recordText = JSON.stringify({
      ...
      assistantSummary: artifact.assistantSummary,
    })
...
export function boundedFusionSourceEvidence(
  entries: readonly EvidenceReference[],
  runID: string,
  sessionID: string,
): EvidenceReference[] {
  const unique = [
  ...
  const selected = unique.slice().sort((left, right) =>
    right.observedAt - left.observedAt || left.id.localeCompare(right.id)
  ).slice(0, 199)
  const omitted = unique.length - selected.length
  return [
    ...selected.sort((left, right) => left.observedAt - right.observedAt || left.id.localeCompare(right.id)).map((
      entry,
    ) => structuredClone(entry)),
    {
      id: `fusion-source-evidence-limit:${
        createHash("sha256").update(runID).update("\0").update(unique.map((entry) => entry.id).join("\0")).digest("hex")
          .slice(0, 32)
      }`,
      kind: "criterion",
      ...
      runID,
      summary:
        `${omitted} older evidence references were omitted from this Fusion source record. Treat unsupported conclusions conservatively; the complete durable evidence ledger remains in the source worktree.`,
    },
  ]
}
...
export function boundedFusionContinuityEvidence(
  ...
  const marker: EvidenceReference = {
    id: `fusion-evidence-limit:${
      createHash("sha256").update(provenanceHash).update("\0").update(targetSessionID).digest("hex").slice(0, 32)
    }`,
    kind: "criterion",
    ...
    summary:
      `${omitted} of ${unique.length} source evidence references were omitted from the compact cross-workspace ledger. Complete deterministic evidence remains in the attached Fusion source records.`,
  }
  const markerBytes = Buffer.byteLength(JSON.stringify(marker))
  while (
    selected.length >= FUSION_CONTINUITY_EVIDENCE_LIMIT || bytes + markerBytes > FUSION_CONTINUITY_EVIDENCE_BYTES
  ) {
  ...
  marker.summary =
    `${finalOmitted} of ${unique.length} source evidence references were omitted from the compact cross-workspace ledger. Complete deterministic evidence remains in the attached Fusion source records.`
