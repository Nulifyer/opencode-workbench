...
import { type GoalVerifierInvocationEnvelope, GoalVerifierInvocationError } from "./goal-verifier-service.js"
...
function tokenCount(value: unknown): number | undefined {
  ...
  const cache = typeof tokens.cache === "object" && tokens.cache !== null && !Array.isArray(tokens.cache)
  ...
  return values.reduce<number>(
    (total, entry) => total + (Number.isSafeInteger(entry) && Number(entry) >= 0 ? Number(entry) : 0),
    0,
  )
}
...
function invocationMetadata(
...
): { sessionID: string; model?: string; tokens?: number; cost?: number } {
  ...
  const cost =
    typeof assistant?.info.cost === "number" && Number.isFinite(assistant.info.cost) && assistant.info.cost >= 0
      ? assistant.info.cost
      : undefined
  return {
    sessionID,
...
export function createOpenCodeGoalVerifierInvocation(
  runtimeProvider: OpenCodeGoalVerifierRuntimeProvider,
...
) => Promise<GoalVerifierInvocationEnvelope> {
  if (!Number.isSafeInteger(pollMilliseconds) || pollMilliseconds < 0 || pollMilliseconds > 10_000) {
    throw new Error("Goal verifier poll interval must be between 0 and 10,000 milliseconds")
  }
  return async (prompt, schema, signal, configuration) => {
    ...
    const onAbort = () => {
      if (!completed) void abortSession()
    }
    signal.addEventListener("abort", onAbort, { once: true })
    if (signal.aborted) onAbort()
    try {
      if (signal.aborted) throw cancellationError(signal, fallbackMetadata)
      await runtime.sendStructuredPrompt(session.id, prompt, {
        agent: configuration?.agent || "plan",
      ...
      while (true) {
        ...
        if (outcome.state === "failed" || outcome.state === "missing") {
          throw new GoalVerifierInvocationError(
            outcome.state === "missing" ? "Goal verifier session no longer exists" : "Goal verifier failed",
            invocationMetadata(messages, session.id, configuration?.model),
          )
        }
        if (outcome.state === "completed") {
          const assistant = messages.slice().reverse().find((message) => message.info.role === "assistant")
          const usedTool = messages.some((message) => message.parts.some((part) => part.type === "tool"))
          const output = usedTool
            ? undefined
            : assistant?.info.structured ?? assistant?.parts.find((part) => part.type === "text" && part.text)?.text
          ...
          throw new GoalVerifierInvocationError(
            usedTool
              ? "Goal verifier attempted to use a forbidden tool"
              : "Goal verifier returned no structured output",
            metadata,
          )
        }
