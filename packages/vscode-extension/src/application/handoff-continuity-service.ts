...
import { type ContextReceipt, type EvidenceReference, sanitizeContextReceipt } from "@opencode-workbench/shared"
...
function sanitizeContinuityReceipt(candidate: ContextReceipt): ContextReceipt {
  ...
  return sanitizeContextReceipt({
    ...
    items: receipt.items.map((item) => ({
      ...
      revision: item.revision === undefined ? undefined : boundedOpaque(item.revision, 256, "Context revision"),
      contentHash: item.contentHash === undefined ? undefined : boundedOpaque(item.contentHash, 256, "Context hash"),
    })),
  })
}
...
function sanitizeContinuityEvidence(
  ...
  return normalizeEvidenceReference({
    ...
    runID: evidence.runID === undefined ? undefined : boundedOpaque(evidence.runID, 1_024, "Evidence run ID"),
...
function compactContinuityEvidence(
  ...
  const marker = previousMarker ?? sanitizeContinuityEvidence({
    ...
    summary:
      "Older evidence references were omitted from the bounded cross-workspace ledger. The complete durable evidence history remains in its source workspace.",
  })
  return [...keep, marker]
}
...
function parseRecord(
  ...
  try {
    ...
    if (
      ![createdAt, updatedAt, expiresAt].every((item) => Number.isSafeInteger(item) && item >= 0) ||
    ...
    const record: HandoffContinuityRecord = {
      ...
      targetDirectory,
      targetSessionID,
      originReceiptIDs: value.originReceiptIDs.map((item) => boundedOpaque(item as string, 1_024, "Origin receipt ID")),
      receipts: value.receipts.map((item) => sanitizeContinuityReceipt(item as ContextReceipt)),
      evidence: value.evidence.map((item) => sanitizeContinuityEvidence(item as EvidenceReference)),
      createdAt,
      updatedAt,
      expiresAt,
    }
    if (
      ...
      record.receipts.some((receipt) => receipt.sessionID !== targetSessionID) ||
...
export class HandoffContinuityService {
  ...
  private async exportHandoffOnce(
    ...
    const originReceiptIDs = [
      ...new Set(
        (input.originReceiptIDs ?? []).map((id) => boundedOpaque(id, 1_024, "Origin receipt ID")),
      ),
    ]
    ...
    return await this.store.transact((current) => {
      ...
      const previous = parsed.registry.records.find((record) => record.id === id)
      const merged: HandoffContinuityRecord = {
        ...
        evidence: compactContinuityEvidence(
          mergeImmutable(
            previous?.evidence ?? [],
            evidence,
            "Evidence reference",
  ...
  async removeHandoff(
    ...
    return await this.enqueueWrite(() => this.removeHandoffOnce(targetDirectory, targetSessionID))
  }
    ...
    while (true) {
      try {
        ...
        try {
          ...
          if (
            processLockCanBeReclaimed(currentOwner, info.mtimeMs, Date.now(), LOCK_STALE_MILLISECONDS, processIsAlive)
          ) {
