...
import { type ReviewDocument, type ReviewFinding, validateReview } from "@opencode-workbench/shared"
    ...
    const findings: ReviewFinding[] = parsed.findings.map((finding) => {
      ...
      return {
        ...
        anchors: finding.anchors.map((anchor) => {
          ...
          ) throw new Error("OpenCode returned an invalid review anchor")
          return {
            file: anchor.file,
            side: anchor.side as "base" | "modified",
            startLine: Number(anchor.startLine),
            endLine: Number(anchor.endLine),
            hunkHeader: typeof anchor.hunkHeader === "string" ? anchor.hunkHeader : undefined,
          }
        }),
      }
    })
