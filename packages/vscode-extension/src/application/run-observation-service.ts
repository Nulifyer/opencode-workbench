...
function tokenTotal(value: unknown): number | undefined {
  ...
  const values = [value.input, value.output, value.reasoning, value.cacheRead ?? cache?.read, value.cacheWrite ?? cache?.write]
  return values.some((entry) => Number.isSafeInteger(entry) && Number(entry) >= 0) ? values.reduce<number>((total, entry) => total + count(entry), 0) : undefined
}

function goalState(messages: MessageBundle[]): string | undefined {
  for (const message of messages.slice().reverse()) {
    for (const part of message.parts.slice().reverse()) {
      if (part.type !== "tool" || !["get_goal", "update_goal", "record_goal_verdict", "configure_goal_verification"].includes(part.tool ?? "") || typeof part.state?.output !== "string" || part.state.output.length > 64 * 1024) continue
      try {
        ...
        const latest = record(goal.latestVerdict) && typeof goal.latestVerdict.verdict === "string" ? goal.latestVerdict.verdict : undefined
...
export function observeRunMessages(messages: MessageBundle[]): ObjectiveRunObservation {
  ...
  for (const message of messages) {
    if (message.info.role !== "assistant" || typeof message.info.cost !== "number" || !Number.isFinite(message.info.cost) || message.info.cost < 0) continue
