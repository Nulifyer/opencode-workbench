...
import { normalizeTaskArtifact, TASK_ARTIFACT_SCHEMA_VERSION, type TaskArtifact } from "@opencode-workbench/shared"
...
export type NewTaskArtifact = TaskArtifact extends infer Artifact
  ? Artifact extends TaskArtifact ? Omit<Artifact, "schemaVersion" | "id" | "revision" | "createdAt" | "updatedAt">
  : never
  : never
...
function mutationID(value: unknown): string {
  if (typeof value !== "string" || !/^[A-Za-z0-9_.:-]{1,1024}$/.test(value)) {
    throw new Error("Task artifact mutation ID is invalid")
  }
  return value
}
...
function normalizeMutation(value: unknown): TaskArtifactMutationRecord {
  ...
  ) throw new Error("Task artifact mutation record is invalid")
  if (!["create", "update", "remove"].includes(String(candidate.operation))) {
    throw new Error("Task artifact mutation operation is invalid")
  }
  if (typeof candidate.fingerprint !== "string" || !/^sha256:[a-f0-9]{64}$/.test(candidate.fingerprint)) {
    throw new Error("Task artifact mutation fingerprint is invalid")
  }
  if (typeof candidate.sessionID !== "string" || !candidate.sessionID || candidate.sessionID.length > 1_024) {
    throw new Error("Task artifact mutation session ID is invalid")
  }
  if (typeof candidate.artifactID !== "string" || !candidate.artifactID || candidate.artifactID.length > 1_024) {
    throw new Error("Task artifact mutation artifact ID is invalid")
  }
  if (typeof candidate.removed !== "boolean") throw new Error("Task artifact mutation removal flag is invalid")
  if (!Number.isSafeInteger(candidate.createdAt) || Number(candidate.createdAt) < 0) {
    throw new Error("Task artifact mutation timestamp is invalid")
  }
  if (
    candidate.resultRevision !== undefined &&
    (!Number.isSafeInteger(candidate.resultRevision) || Number(candidate.resultRevision) < 1)
  ) throw new Error("Task artifact mutation result revision is invalid")
  if (candidate.removed === (candidate.resultRevision !== undefined)) {
    throw new Error("Task artifact mutation result is inconsistent")
  }
  return {
...
export class TaskArtifactService {
  ...
  get(sessionID: string, artifactID: string): TaskArtifact | undefined {
    const artifact = this.state.artifacts.find((candidate) =>
      candidate.sessionID === sessionID && candidate.id === artifactID
    )
  ...
  create(input: NewTaskArtifact, requestMutationID?: string): TaskArtifact {
    ...
    if (this.state.artifacts.length >= TASK_ARTIFACT_CAPACITY) {
      throw new Error(`Task artifact capacity of ${TASK_ARTIFACT_CAPACITY} has been reached`)
    }
    const now = Date.now()
    ...
    if (requestMutationID !== undefined) {
      this.addMutation(next, {
        id: mutationID(requestMutationID),
        operation: "create",
        fingerprint: operationFingerprint,
        sessionID: artifact.sessionID,
        artifactID: artifact.id,
        resultRevision: artifact.revision,
        removed: false,
        createdAt: now,
      })
    }
  ...
  update(
    ...
    if (!Number.isSafeInteger(expectedRevision) || expectedRevision < 1) {
      throw new Error("Expected task artifact revision is invalid")
    }
    const index = this.state.artifacts.findIndex((candidate) =>
      candidate.sessionID === sessionID && candidate.id === artifactID
    )
    if (index < 0) throw new Error("Task artifact was not found")
    const current = this.state.artifacts[index]!
    if (current.revision !== expectedRevision) {
      throw new Error(`Task artifact revision is stale; expected ${current.revision}`)
    }
  ...
  archive(sessionID: string, artifactID: string, expectedRevision: number, requestMutationID?: string): TaskArtifact {
    return this.update(sessionID, artifactID, expectedRevision, (artifact) => {
  ...
  private restore(raw: unknown): void {
    ...
    if (
      !source || !exactKeys(source, ["schemaVersion", "artifacts", "mutations"]) ||
      source.schemaVersion !== TASK_ARTIFACT_SCHEMA_VERSION || !Array.isArray(source.artifacts) ||
    ...
    for (let index = 0; index < source.artifacts.length; index += 1) {
      try {
        if (restored.artifacts.length >= TASK_ARTIFACT_CAPACITY) {
          throw new Error(`capacity exceeds ${TASK_ARTIFACT_CAPACITY}`)
        }
        ...
        this.reportProblem(
          `Ignored corrupt task artifact at index ${index}: ${error instanceof Error ? error.message : String(error)}`,
        )
      }
    }
    ...
    for (let index = 0; index < source.mutations.length; index += 1) {
      try {
        if (restored.mutations.length >= TASK_ARTIFACT_MUTATION_CAPACITY) {
          throw new Error(`mutation capacity exceeds ${TASK_ARTIFACT_MUTATION_CAPACITY}`)
        }
        ...
        if (!mutation.removed && !artifactIDs.has(mutation.artifactID)) {
          throw new Error("mutation references a missing artifact")
        }
        ...
        this.reportProblem(
          `Ignored corrupt task artifact mutation at index ${index}: ${
  ...
  private replay(
    requestMutationID: string,
    operation: TaskArtifactMutationRecord["operation"],
    ...
    const artifact = this.state.artifacts.find((candidate) =>
      candidate.id === mutation.artifactID && candidate.sessionID === mutation.sessionID
    )
    if (!artifact) throw new Error("Task artifact mutation references a missing result")
    return clone(artifact)
  }

  private replayMutation(
    requestMutationID: string,
    operation: TaskArtifactMutationRecord["operation"],
    operationFingerprint: string,
  ): TaskArtifactMutationRecord | undefined {
    const id = mutationID(requestMutationID)
    const previous = this.state.mutations.find((candidate) => candidate.id === id)
    if (!previous) return undefined
    if (previous.operation !== operation || previous.fingerprint !== operationFingerprint) {
      throw new Error("Task artifact mutation ID was reused for a different operation")
    }
    return previous
  }

  private addMutation(state: TaskArtifactState, mutation: TaskArtifactMutationRecord): void {
    if (state.mutations.length >= TASK_ARTIFACT_MUTATION_CAPACITY) {
      throw new Error(`Task artifact mutation capacity of ${TASK_ARTIFACT_MUTATION_CAPACITY} has been reached`)
    }
    state.mutations.push(normalizeMutation(mutation))
  }

  private commit(next: TaskArtifactState): void {
    if (next.artifacts.length > TASK_ARTIFACT_CAPACITY) {
      throw new Error(`Task artifact capacity of ${TASK_ARTIFACT_CAPACITY} has been exceeded`)
    }
  ...
  private assertSize(state: TaskArtifactState): void {
    if (encodedBytes(state) > TASK_ARTIFACT_STATE_BYTE_LIMIT) {
      throw new Error(`Task artifact state exceeds the ${TASK_ARTIFACT_STATE_BYTE_LIMIT}-byte limit`)
    }
  }
