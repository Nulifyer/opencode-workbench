...
const PROJECTION_MESSAGE =
  "Workbench limited this view to stay within the webview transport budget. Some history or catalog entries are temporarily hidden; the underlying sessions and durable records were not deleted."
...
function omissionTotals(snapshot: ChatSnapshot): MutableOmissions {
  ...
  return {
    ...
    runtimeServices: (snapshot.runtime?.lsp.length ?? 0) + (snapshot.runtime?.formatters.length ?? 0) +
      (snapshot.runtime?.mcp.length ?? 0),
...
function minimalSnapshot(
  ...
  const selectedModel = source?.model
    ? snapshot.models.find((model) => `${model.providerID}/${model.id}` === source.model)
    : undefined
...
function taskArtifactRank(artifact: TaskArtifactSummary): number {
  if (
    artifact.stale ||
    ["failed", "unavailable", "blocked", "needs-user", "incomplete", "limited"].includes(artifact.state)
  ) return 0
...
function exactOmissions(omissions: MutableOmissions): ChatSnapshotProjectionOmissions {
  return Object.fromEntries(
    Object.entries(omissions).filter(([, count]) => count > 0),
  ) as ChatSnapshotProjectionOmissions
}
...
export function projectChatSnapshotForWebview(
  ...
  if (sourceSession && projectedSession) {
    if (sourceSession.queue && projectedSession.queue) {
      ...
      if (
        sourceSession.inFlightPromptID &&
        projectedSession.queue.some((prompt) => prompt.id === sourceSession.inFlightPromptID)
      ) {
    ...
    if (sourceSession.permissions && projectedSession.permissions) {
      admitArray(
        ...
        projectedSession.permissions,
        sourceSession.permissions.map((_, index) => index),
        "permissions",
      )
    }
    if (sourceSession.questions && projectedSession.questions) {
      admitArray(
        sourceSession.questions,
        projectedSession.questions,
        sourceSession.questions.map((_, index) => index),
        "questions",
      )
    }
    ...
    projectedSession.messageRevisions = Object.fromEntries(
      projectedSession.messages.map((
        message,
      ) => [message.info.id, sourceSession.messageRevisions[message.info.id] ?? 0]),
    )
    projectedSession.history = omissions.messages
      ? projectedHistory(sourceSession, projectedSession.messages.length)
      : sourceSession.history
  }
  ...
  const artifactOrder = snapshot.artifacts
    ? descendingIndexes(
      snapshot.artifacts,
      (left, right) =>
        taskArtifactRank(left) - taskArtifactRank(right) || right.updatedAt - left.updatedAt ||
        left.id.localeCompare(right.id),
    )
    : []
  if (snapshot.artifacts && candidate.artifacts) {
    admitArray(snapshot.artifacts, candidate.artifacts, artifactOrder.slice(0, 1), "taskArtifacts")
  }
  const reviewSeverity = { critical: 0, high: 1, medium: 2, low: 3 } as const
  const reviewFindingOrder = snapshot.reviewFindings
    ? descendingIndexes(
      snapshot.reviewFindings,
      (left, right) =>
        Number(left.disposition !== "open") - Number(right.disposition !== "open") ||
    ...
    : []
  if (snapshot.reviewFindings && candidate.reviewFindings) {
    admitArray(snapshot.reviewFindings, candidate.reviewFindings, reviewFindingOrder.slice(0, 1), "reviewFindings")
  }
  const comparisonOrder = snapshot.runComparisons
    ? descendingIndexes(
      snapshot.runComparisons,
      (left: RunComparisonSnapshot, right: RunComparisonSnapshot) =>
        right.updatedAt - left.updatedAt || left.artifactID.localeCompare(right.artifactID),
    )
    : []
  if (snapshot.runComparisons && candidate.runComparisons) {
    admitArray(snapshot.runComparisons, candidate.runComparisons, comparisonOrder.slice(0, 1), "runComparisons")
  }
  const evidenceOrder = snapshot.evidence
    ? descendingIndexes(
      snapshot.evidence,
      (left, right) =>
        evidenceRank(left) - evidenceRank(right) || right.observedAt - left.observedAt ||
        left.id.localeCompare(right.id),
    )
    : []
  if (snapshot.evidence && candidate.evidence) {
    admitArray(snapshot.evidence, candidate.evidence, evidenceOrder.slice(0, 1), "evidence")
  }
  ...
  const runGroupOrder = snapshot.runGroups
    ? descendingIndexes(
      snapshot.runGroups,
      (left, right) =>
        runGroupRank(left) - runGroupRank(right) || right.createdAt - left.createdAt || left.id.localeCompare(right.id),
    )
    : []
  if (snapshot.runGroups && candidate.runGroups) {
    admitArray(snapshot.runGroups, candidate.runGroups, runGroupOrder.slice(0, 1), "runGroups")
  }
  const firstReferencedWorktreeIDs = new Set(
    candidate.runGroups?.flatMap((group) => group.runs.flatMap((run) => run.worktreeID ? [run.worktreeID] : [])) ?? [],
  )
  const firstWorktreeOrder = snapshot.worktrees
    ? descendingIndexes(
      snapshot.worktrees,
      (left, right) =>
        worktreeRank(left, firstReferencedWorktreeIDs) - worktreeRank(right, firstReferencedWorktreeIDs) ||
        right.updatedAt - left.updatedAt || left.id.localeCompare(right.id),
    )
    : []
  if (snapshot.worktrees && candidate.worktrees) {
    admitArray(snapshot.worktrees, candidate.worktrees, firstWorktreeOrder.slice(0, 1), "worktrees")
  }
  const ptyOrder = snapshot.ptys
    ? descendingIndexes(
      snapshot.ptys,
      (left, right) =>
        Number(right.status === "running") - Number(left.status === "running") || left.id.localeCompare(right.id),
    )
    : []
  ...
  const lineageOrder = snapshot.lineage
    ? descendingIndexes(snapshot.lineage, (left, right) => {
      ...
      return leftSelected - rightSelected || leftAttention - rightAttention || left.depth - right.depth ||
        right.updatedAt - left.updatedAt || left.sessionID.localeCompare(right.sessionID)
    })
    : []
  if (snapshot.lineage && candidate.lineage) {
    admitArray(snapshot.lineage, candidate.lineage, lineageOrder.slice(0, 1), "lineage")
  }
  const receiptOrder = sourceSession?.contextReceipts
    ? descendingIndexes(
      sourceSession.contextReceipts,
      (left, right) => right.admittedAt - left.admittedAt || left.id.localeCompare(right.id),
    )
    : []
  if (sourceSession?.contextReceipts && projectedSession?.contextReceipts) {
    admitArray(
      sourceSession.contextReceipts,
      projectedSession.contextReceipts,
      receiptOrder.slice(0, 1),
      "contextReceipts",
    )
  }
  ...
  if (snapshot.walkthroughs && candidate.walkthroughs) {
    ...
    walkthroughOrder = descendingIndexes(
      snapshot.walkthroughs,
      (left, right) => right.generatedAt - left.generatedAt || left.id.localeCompare(right.id),
    )
    admitWalkthroughs = (order): void => {
      ...
      destination.splice(
        0,
        destination.length,
        ...snapshot.walkthroughs!.flatMap((_, index) => retained.has(index) ? [retained.get(index)!] : []),
      )
    }
  ...
  if (snapshot.artifacts && candidate.artifacts) {
    admitArray(snapshot.artifacts, candidate.artifacts, artifactOrder, "taskArtifacts")
  }
  if (snapshot.reviewFindings && candidate.reviewFindings) {
    admitArray(snapshot.reviewFindings, candidate.reviewFindings, reviewFindingOrder, "reviewFindings")
  }
  if (snapshot.runComparisons && candidate.runComparisons) {
    admitArray(snapshot.runComparisons, candidate.runComparisons, comparisonOrder, "runComparisons")
  }
  if (snapshot.evidence && candidate.evidence) {
    admitArray(snapshot.evidence, candidate.evidence, evidenceOrder, "evidence")
  }
  if (snapshot.runGroups && candidate.runGroups) {
    admitArray(snapshot.runGroups, candidate.runGroups, runGroupOrder, "runGroups")
  }
  const retainedWorktreeIDs = new Set(
    candidate.runGroups?.flatMap((group) => group.runs.flatMap((run) => run.worktreeID ? [run.worktreeID] : [])) ?? [],
  )
  if (snapshot.worktrees && candidate.worktrees) {
    const order = descendingIndexes(
      snapshot.worktrees,
      (left, right) =>
        worktreeRank(left, retainedWorktreeIDs) - worktreeRank(right, retainedWorktreeIDs) ||
        right.updatedAt - left.updatedAt || left.id.localeCompare(right.id),
    )
  ...
  if (sourceSession && projectedSession) {
    ...
    if (sourceSession.changes && projectedSession.changes) {
      admitArray(
        sourceSession.changes,
        projectedSession.changes,
        sourceSession.changes.map((_, index) => index),
        "changes",
      )
    }
  }
  ...
  const sessionOrder = descendingIndexes(snapshot.sessions, (left, right) => {
    const leftAttention = Boolean(left.attention) || ["busy", "retry", "error"].includes(left.status.type) ? 0 : 1
    const rightAttention = Boolean(right.attention) || ["busy", "retry", "error"].includes(right.status.type) ? 0 : 1
    return leftAttention - rightAttention || (right.updatedAt ?? 0) - (left.updatedAt ?? 0) ||
      left.id.localeCompare(right.id)
  })
  ...
  if (snapshot.attentionItems && candidate.attentionItems) {
    admitArray(
      ...
      candidate.attentionItems,
      snapshot.attentionItems.map((_, index) => index),
      "attentionItems",
    )
  }
  ...
  if (snapshot.mentionAgents && candidate.mentionAgents) {
    admitArray(
      snapshot.mentionAgents,
      candidate.mentionAgents,
      snapshot.mentionAgents.map((_, index) => index),
      "catalogItems",
    )
  }
  if (snapshot.providers && candidate.providers) {
    admitArray(
      snapshot.providers,
      candidate.providers,
      snapshot.providers.map((_, index) => index),
      "catalogItems",
    )
  }
  admitArray(snapshot.models, candidate.models, snapshot.models.map((_, index) => index), "catalogItems")
  if (snapshot.resources && candidate.resources) {
    admitArray(
      snapshot.resources,
      candidate.resources,
      snapshot.resources.map((_, index) => index),
      "catalogItems",
    )
  }
  if (snapshot.commands && candidate.commands) {
    admitArray(
      snapshot.commands,
      candidate.commands,
      snapshot.commands.map((_, index) => index),
      "catalogItems",
    )
  }
