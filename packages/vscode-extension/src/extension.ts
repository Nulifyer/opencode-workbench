...
import { type ComposerPreferences, SessionController } from "./session-controller.js"
...
} from "./application/plan-service.js"
...
import { type DiffCapture, diffFileChangeKind, DiffService } from "./application/diff-service.js"
...
import {
  ...
  diagnosticsSummary,
} from "./application/diagnostics-evidence-service.js"
...
import { type AttentionReadRecord, AttentionReadService } from "./application/attention-read-service.js"
import { ChangeReviewService, type ReviewedChangeRecord } from "./application/change-review-service.js"
...
const CHANGE_REVIEWS_STATE = "opencodeWorkbench.changeReviews.v1"
...
async function executeInTerminal(
  ...
  executable: string,
  args: string[],
): Promise<vscode.TerminalShellExecution> {
  terminal.show()
...
async function reloadController(controller: SessionController, request: OpenCodeReloadRequest): Promise<void> {
  ...
  const restoreID = selectedID && Object.hasOwn(controller.snapshot.sessions, selectedID)
    ? selectedID
    : request.sessionID
  if (
    restoreID && Object.hasOwn(controller.snapshot.sessions, restoreID) && controller.snapshot.selectedID !== restoreID
  ) {
...
async function waitForAssistantResult(
  ...
  while (Date.now() < deadline) {
    ...
    if (status?.type === "error" || assistantTurnFailed(messages)) {
      throw new Error("OpenCode failed while generating the plan")
    }
...
async function currentDebugContext(repository: string): Promise<string> {
  ...
  const breakpoints = (await Promise.all(
    vscode.debug.breakpoints.slice(0, 100).map(async (breakpoint) => {
  ...
  return JSON.stringify(
    {
      activeSession: active && activeFolder !== undefined
        ? {
          name: active.name.slice(0, 500),
          type: active.type.slice(0, 200),
          workspaceFolder: activeFolder.replaceAll(path.sep, "/"),
        }
        : undefined,
      sourceBreakpoints: breakpoints,
      sourceBreakpointCount:
        vscode.debug.breakpoints.filter((entry) => entry instanceof vscode.SourceBreakpoint).length,
...
async function invokeBoundedOpenCode(
  runtime: OpenCodeClient,
  title: string,
  prompt: string,
  model: string,
  ...
  timeoutMilliseconds = 300_000,
): Promise<BoundedOpenCodeResult> {
  const session = await runtime.createSession(title)
  await runtime.sendPrompt(session.id, createOpenCodeMessageID(), prompt, "steer", "plan", model, undefined, [{
    type: "file",
    ...file,
  }])
  const deadline = Date.now() + timeoutMilliseconds
...
async function nativeDiffText(
  diffs: DiffService,
  capture: DiffCapture,
  filePath: string,
): Promise<{ base: string; modified: string; basePath: string; modifiedPath: string }> {
  if (!capture.snapshot.complete) {
    throw new Error(`Diff is incomplete: ${capture.snapshot.truncationReason ?? "unknown limitation"}`)
  }
  ...
  if (kind !== "deleted") {
    if (capture.snapshot.headRef) modified = await diffs.readRevisionText(root, capture.snapshot.headRef, modifiedPath)
      ...
      if (parentRelative === ".." || parentRelative.startsWith(`..${path.sep}`) || path.isAbsolute(parentRelative)) {
        throw new Error("Modified-side diff path escapes the repository")
      }
...
async function openDiffAnchor(
  ...
  const editor = vscode.window.visibleTextEditors.find((candidate) =>
    candidate.document.uri.toString() === targetUri.toString()
  )
...
async function startIsolatedTask(client: OpenCodeClient, worktrees: WorktreeService, workspacePath: string, input: {
  ...
  const entry = await worktrees.create({
    directory: repository.root,
    path: path.join(path.dirname(repository.root), `.${path.basename(repository.root)}-worktrees`, identity.name),
  ...
  try {
    ...
    await worktrees.failDurably(entry.id, {
      code: "INTERNAL",
      message: promptID
        ? "OpenCode prompt admission failed for this isolated worktree"
        : "OpenCode session creation failed for this isolated worktree",
      retryable: true,
    })
    throw error
  }
...
function currentDiagnostics(repository: string): DiagnosticsSnapshot {
  return captureDiagnostics(
    ...
    vscode.languages.getDiagnostics().flatMap(([uri, diagnostics]) => {
      ...
      const fileErrors = diagnostics.filter((diagnostic) =>
        diagnostic.severity === vscode.DiagnosticSeverity.Error
      ).length
      const fileWarnings =
        diagnostics.filter((diagnostic) => diagnostic.severity === vscode.DiagnosticSeverity.Warning).length
...
async function externalConnection(context: vscode.ExtensionContext, directory: string): Promise<OpenCodeConnection> {
  ...
  const environment = await serverEnvironment(
    configuration.get<string>("serverEnvironmentFile", "~/.config/opencode-workbench/server.env"),
  )
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  if (folders.length > 1) {
    ...
    const selected = folders.find((folder) => folder.uri.toString() === remembered) ??
      await vscode.window.showWorkspaceFolderPick({ placeHolder: "Choose the workspace root for OpenCode Workbench" })
    if (selected) workspaceFolder = selected
    if (workspaceFolder) {
      await context.workspaceState.update("opencodeWorkbench.workspaceRoot", workspaceFolder.uri.toString())
    }
  }
  ...
  if (!workspacePath) {
    const provider = new ChatViewProvider(
      ...
      undefined,
      undefined,
      "Open a trusted workspace folder to use OpenCode Workbench",
      () => output.show(true),
      report,
    )
    const participant = vscode.chat.createChatParticipant(
      "opencodeWorkbench.opencode",
      async (request, _chatContext, stream) => {
        if (request.command === "open") {
          await vscode.commands.executeCommand("opencodeWorkbench.openChat")
          stream.markdown(
            "Opened OpenCode Workbench. Open a trusted workspace folder before starting an OpenCode session.",
          )
          return
        }
        stream.markdown(
          "Open a trusted workspace folder before handing a task to OpenCode. No model request was run in VS Code Chat.",
        )
      },
    )
    ...
    context.subscriptions.push(
      provider,
      participant,
      vscode.window.registerWebviewPanelSerializer("opencodeWorkbench.chatEditor", {
        deserializeWebviewPanel: async (panel) => provider.restoreEditor(panel),
      }),
      vscode.window.registerWebviewViewProvider("opencodeWorkbench.chat", provider),
      vscode.commands.registerCommand(
        "opencodeWorkbench.openChat",
        async () => vscode.commands.executeCommand("opencodeWorkbench.chat.focus"),
      ),
      ...
      vscode.commands.registerCommand(
        ...
        () =>
          vscode.commands.executeCommand(
            ...
            "nulifyer.opencode-workbench#opencodeWorkbench.gettingStarted",
            false,
          ),
      ),
    )
  ...
  const changeReviews = new ChangeReviewService(
    context.workspaceState.get<ReviewedChangeRecord[]>(CHANGE_REVIEWS_STATE) ?? [],
    (records) => stateWriter.write(CHANGE_REVIEWS_STATE, records),
  )
  context.subscriptions.push(changeReviews)
  ...
  const queueContinuityEvidence = (entry: EvidenceReference): void => {
    if (
      !handoffContinuity || !entry.sessionID || !continuitySessions.has(entry.sessionID) ||
      publishedContinuityEvidence.has(entry.id)
    ) return
    ...
    handoffContinuity.queueHandoff({
      targetDirectory: canonicalWorkspace,
  ...
  const queueLatestContinuityEvidence = (entries: readonly EvidenceReference[]): void => {
    ...
    for (const values of grouped.values()) {
      for (
        const entry of values.sort((left, right) =>
          right.observedAt - left.observedAt || left.id.localeCompare(right.id)
        ).slice(0, 199)
      ) queueContinuityEvidence(entry)
    }
  }
  ...
  const requireContinuity = (): HandoffContinuityService => {
    if (!handoffContinuity) {
      throw new Error(
        "Private cross-workspace continuity is unavailable; isolated Workbench workflows are disabled for this repository",
      )
    }
  ...
  const publishIsolatedHandoff = async (
    entry: WorktreeJournalEntry,
    originReceiptIDs: string[] = [],
    extraEvidence: EvidenceReference[] = [],
  ): Promise<void> => {
    ...
    await continuity.exportHandoff({
      targetDirectory,
      targetSessionID: entry.sessionID,
      trackingOnly: !receipt && !extraEvidence.length,
      originReceiptIDs,
  ...
  const publishRunContinuity = async (group: RunGroup, runID?: string): Promise<void> => {
    const continuity = requireContinuity()
    await flushDurableMetadata()
    await Promise.all(
      group.runs.filter((run) => !runID || run.id === runID).flatMap((run) => {
        ...
        if (!journal?.promptID || !receipt) {
          throw new Error(`Run ${run.id} did not commit its per-message context receipt`)
        }
        return [
          fs.realpath(run.session.directory).then((targetDirectory) =>
            continuity.exportHandoff({
              targetDirectory,
              targetSessionID: run.session.sessionID,
              originReceiptIDs: [group.promptReceiptID],
              receipts: [receipt],
            })
          ),
        ]
  ...
  const collectRunObservations = async (
    group: RunGroup,
  ): Promise<Record<string, ReturnType<typeof observeRunMessages> & { evidence: EvidenceReference[] }>> => {
    if (!client) return {}
    const entries = await Promise.all(group.runs.map(async (run) => {
      if (handoffContinuity && run.session.sessionID !== "pending") {
        const imported = await handoffContinuity.importHandoff(
          await fs.realpath(run.session.directory),
      ...
      const messages = run.session.sessionID === "pending"
        ? []
        : await client!.forDirectory(run.session.directory).messages(run.session.sessionID).catch(() => [])
      const scoped = [
        ...evidence.list({ sessionID: run.session.sessionID }),
        ...evidence.list({ runGroupID: group.id, runID: run.id }),
      ]
...

  {
    activeBridge = new VsCodeBridge(workspacePath, {
      requestOpenCodeReload: mode === "managed"
        ? (request) => {
      ...
      terminalEvidence: ({ sessionID, exitCode }) => {
        evidence.record({
          kind: "terminal",
          label: "OpenCode terminal command",
          status: exitCode === 0 ? "passed" : exitCode === undefined ? "unknown" : "failed",
          sessionID,
          repository: workspacePath,
          summary: exitCode === undefined
            ? "Terminal execution ended without a reported exit code"
            : `Terminal execution exited with code ${exitCode}`,
        })
      },
      taskEvidence: ({ sessionID, name, source, group, exitCode }) => {
        evidence.record({
          kind: group === vscode.TaskGroup.Test.id ? "test" : "task",
          label: name,
          status: exitCode === 0 ? "passed" : exitCode === undefined ? "unknown" : "failed",
          sourceID: source,
          sessionID,
          repository: workspacePath,
          summary: exitCode === undefined
            ? "Task ended without a reported exit code"
    ...
    try {
      ...
      goalVerifier = new GoalVerifierService(
        createOpenCodeGoalVerifierInvocation(() => client!.forDirectory(workspacePath)),
      )
      multiRun = new MultiRunOrchestrator(runGroups, worktrees, {
        forDirectory: (directory) => {
          ...
          return {
            ...
            needsInput: async (sessionID) => {
              ...
              return permissions.some((request) => request.sessionID === sessionID) ||
                questions.some((request) => request.sessionID === sessionID)
            },
        ...
        onRunAdmitted: async (groupID, runID) => {
          const group = runGroups.get(groupID)
          if (!group) return
          try {
            await publishRunContinuity(group, runID)
          } catch (error) {
            report(`Run ${runID} started, but continuity export failed: ${userFacingError(error)}`)
            void vscode.window.showWarningMessage(
              "An isolated run started, but its cross-worktree continuity metadata could not be exported. The run remains active; do not start it again.",
            )
          }
        },
        admission: {
          ...
          admit: (sessionID, promptID) => {
            ...
            const journal = receipt
              ? worktrees.journal().find((entry) => entry.sessionID === sessionID && entry.promptID === promptID)
              : undefined
            if (receipt && journal && handoffContinuity) {
              handoffContinuity.queueHandoff({
                targetDirectory: journal.path,
                targetSessionID: sessionID,
                originReceiptIDs: sourceReceiptID ? [sourceReceiptID] : [],
                receipts: [receipt],
              })
            }
          },
      ...
      void Promise.all(
        runGroups.list().map(async (group) => {
      ...
      const validateExternalVersion = async (): Promise<void> => {
        ...
        if (!supportedVersion(health.version)) {
          throw new Error(
            `OpenCode ${health.version} is not supported; install a compatible 1.18.x release at or above 1.18.11`,
          )
        }
      }
      if (mode === "external") {
      ...
      const preferences = globalPreferences ??
        context.workspaceState.get<ComposerPreferences>("opencodeWorkbench.composerPreferences")
      if (!globalPreferences && preferences) {
        await context.globalState.update("opencodeWorkbench.composerPreferences", preferences)
      }
        ...
        client,
        {
          error: (message) => {
            trace.record({
              ...
              error: "Controller operation failed; inspect the extension output for sanitized operational details",
            })
            report(message)
          },
          preferencesChanged: (preferences) =>
            void context.globalState.update("opencodeWorkbench.composerPreferences", preferences),
          ...
          sessionRecovered: (sourceSessionID, recoveredSessionID) => {
            trace.record({
              ...
              requestID: sourceSessionID,
              transition: "legacy->compatible-fork",
            })
            const recovered = context.workspaceState.get<Record<string, string>>(RECOVERED_SESSIONS_STATE) ?? {}
            void context.workspaceState.update(RECOVERED_SESSIONS_STATE, {
              ...recovered,
              [sourceSessionID]: recoveredSessionID,
            })
          },
          promptAdmitted: (sessionID, promptID, admittedAt) => {
            trace.record({
              type: "admission.committed",
              sessionID,
              requestID: promptID,
              timestamp: admittedAt,
              transition: "pending->admitted",
            })
            ...
            if (directory) {
              void diffs.captureTurnBaseline(directory).then((baseline) =>
                turnBaselines.set(sessionID, { promptID, ...baseline })
              ).catch((error) => report(`Could not capture turn diff baseline: ${errorMessage(error)}`))
            }
          },
          ...
          openExternal: async (value) => {
            ...
            if (url.protocol !== "http:" && url.protocol !== "https:") {
              throw new Error("OpenCode requested an unsupported external URL")
            }
            const choice = await vscode.window.showWarningMessage(
              "OpenCode could not launch the MCP authentication page.",
              "Open in browser",
            )
            if (choice !== "Open in browser") return
            if (!await vscode.env.openExternal(vscode.Uri.parse(url.toString()))) {
              throw new Error("VS Code did not open the external URL")
            }
          },
        },
        preferences,
        context.workspaceState.get<string>(SELECTED_SESSION_STATE),
        context.workspaceState.get<Record<string, string>>(RECOVERED_SESSIONS_STATE),
      )
      controller.start()
  ...
  const openRawHealth = async (): Promise<void> => {
    ...
    const document = await vscode.workspace.openTextDocument({
      language: "json",
      content: JSON.stringify(health.snapshot(), null, 2),
    })
  ...
  chatProvider = new ChatViewProvider(
    ...
    (observation) => trace.record(observation),
    {
      ...
      trace: () =>
        trace.snapshot().slice(-100).map((entry) => ({
          type: entry.type,
      ...
      captureBrowserContext: async (request) => {
        await vscode.commands.executeCommand("opencodeWorkbench.captureBrowserContext", request)
      },
      startMultiModel: async (request) => {
        if (!multiRun) throw new Error("OpenCode Multi-run is unavailable")
        if (request.models.length < 2 || request.models.length > MULTI_RUN_MAX_CANDIDATES) {
          throw new Error(`Select between 2 and ${MULTI_RUN_MAX_CANDIDATES} models`)
        }
        if (
          request.concurrency < 1 || request.concurrency > MULTI_RUN_MAX_CONCURRENCY ||
          request.concurrency > request.models.length
        ) {
          throw new Error(
            `Concurrent runs must be between 1 and ${Math.min(MULTI_RUN_MAX_CONCURRENCY, request.models.length)}`,
          )
        }
        const available = new Set(
          controller!.chatSnapshot().models.map((candidate) => `${candidate.providerID}/${candidate.id}`),
        )
        if (request.models.some((candidate) => !available.has(candidate))) {
          throw new Error("The model catalog changed; review the selected candidates and try again")
        }
        const repository = await worktrees.repository(workspacePath)
        const baseRef = (await gitRunner.run(["rev-parse", "HEAD"], repository.root)).stdout.trim()
        const mutationID = randomUUID()
        contextReceipts.stage(
          request.ownerSessionID,
          mutationID,
          request.receiptItems,
          request.receiptItems.some((item) => item.truncated) ? "explicit" : "none",
        )
  ...
  const telemetrySubscription = controller?.subscribe((update) => {
    ...
    const sessionID = "sessionID" in update && typeof update.sessionID === "string"
      ? update.sessionID
      : event && typeof event.properties.sessionID === "string"
      ? event.properties.sessionID
      : undefined
    const transition = event?.type === "session.status" && typeof event.properties.status === "object" &&
        event.properties.status !== null && "type" in event.properties.status
      ? String(event.properties.status.type).slice(0, 256)
      : update.type === "permissions"
      ? `${update.permissions.length} pending`
      : update.type === "questions"
      ? `${update.questions.length} pending`
      : update.type === "queue"
      ? update.prompt.delivery
      : undefined
    ...
    if (update.type === "changes") {
      ...
      if (directory && baseline) {
        void diffs.capture({
          ...
          scope: "turn",
          baseRef: baseline.headRef,
          baselineClean: baseline.clean,
        }).then((capture) => {
          ...
          trace.record({
            ...
            requestID: baseline.promptID,
            diffHash: capture.snapshot.unifiedDiffHash,
            transition: capture.snapshot.complete ? "complete" : "incomplete",
          })
  ...
  const run =
    <TArgs extends unknown[]>(operation: (...args: TArgs) => Promise<unknown>) =>
    async (...args: TArgs): Promise<void> => {
      ...
      try {
        ...
        trace.record({
          ...
          error: "Command failed; inspect the extension output for operational details",
        })
        report(errorMessage(error))
        if (!isUserCancellation(error)) {
          await vscode.window.showErrorMessage(`OpenCode Workbench: ${userFacingError(error)}`)
        }
      }
    }
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand(
      "opencodeWorkbench.taskArtifactAction",
      run(async (raw: unknown) => {
        ...
        if (
          typeof action.sessionID !== "string" || typeof action.artifactID !== "string" ||
          typeof action.action !== "string"
        ) throw new Error("Invalid task artifact action")
        ...
        const expectedRevision = Number.isSafeInteger(action.expectedRevision)
          ? Number(action.expectedRevision)
          : artifact.revision
        if (action.action === "set-finding-disposition") {
          if (
            artifact.kind !== "review" || typeof action.findingID !== "string" ||
            !["open", "fixed", "dismissed", "accepted-risk"].includes(String(action.disposition))
          ) throw new Error("Invalid review finding disposition")
          const findingID = action.findingID
          if (!artifact.payload.document.findings.some((finding) => finding.id === findingID)) {
            throw new Error("Review finding is no longer available")
          }
          const capture = await diffs.capture({
            repository: artifact.payload.repository,
            scope: artifact.payload.scope,
            baseRef: artifact.payload.baseRef,
          })
          if (
            !capture.snapshot.complete || capture.snapshot.unifiedDiffHash !== artifact.payload.diffHash ||
            artifact.payload.stale
          ) {
        ...
        if (action.action === "regenerate") {
          ...
          await vscode.commands.executeCommand(
            ...
            artifact.payload.repository,
            artifact.payload.baseRef,
            artifact.sessionID,
          )
          return
        }
        if (action.action === "approve") {
          ...
          if (document.uri.scheme === "untitled") {
            throw new Error("Save the plan to a user-owned Markdown file before approving it")
          }
          const storedUri = vscode.Uri.parse(artifact.payload.uri, true)
          if (storedUri.scheme !== "untitled" && document.uri.toString() !== artifact.payload.uri) {
            throw new Error("Open the plan document before approving it")
          }
          const planSessionID = /^> OpenCode session:\s*(\S+)\s*$/m.exec(document.getText())?.[1]
          if (planSessionID !== artifact.sessionID) {
            throw new Error("The open Markdown document does not belong to this OpenCode plan session")
          }
        ...
        const openArtifact = async (): Promise<void> => {
          if (artifact.kind === "plan") {
            ...
            const result = messages.slice().reverse().find((message) => message.info.role === "assistant")?.parts
              .filter((part) => part.type === "text" && part.text).map((part) => part.text).join("\n").trim()
          ...
          if (artifact.kind === "review") {
            ...
            const markdown = [
              ...
              ``,
              `Diff: \`${review.diffHash}\`${artifact.payload.stale ? " · **STALE**" : ""}`,
              ``,
              ...(review.findings.length
                ? review.findings.flatMap((
                  finding,
                ) => [
                  ...
                  ...finding.anchors.map((anchor) =>
                    `- ${anchor.file}:${anchor.startLine}-${anchor.endLine} (${anchor.side})`
                  ),
                  ``,
                ])
            ...
            await vscode.window.showTextDocument(
              await vscode.workspace.openTextDocument({ language: "markdown", content: markdown }),
              { preview: false },
            )
            return
          }
          if (artifact.kind === "run-comparison") {
            ...
            const content = group
              ? runComparison.markdown(group, artifact.payload.rows)
              : JSON.stringify(artifact.payload, null, 2)
            await vscode.window.showTextDocument(
              await vscode.workspace.openTextDocument({ language: group ? "markdown" : "json", content }),
              { preview: false },
            )
            return
          }
    ...
    vscode.commands.registerCommand(
      ...
      run(async (uri?: vscode.Uri, selected?: vscode.Uri[]) => {
    ...
    vscode.commands.registerCommand(
      ...
      run(async () => {
        ...
        const objective = await vscode.window.showInputBox({
          title: "Plan Task",
          prompt: "What should OpenCode plan?",
          ignoreFocusOut: true,
          validateInput: (value) => value.trim() ? undefined : "Enter a task to plan",
        })
        if (!objective) return
        ...
        void waitForAssistantResult(client.forDirectory(workspacePath), sessionID).then(async (result) => {
          ...
          if (generatedPlanDisposition(initialVersion, document.version) === "preserve-user-draft") {
            ...
            void vscode.window.showInformationMessage(
              "Your edited plan draft was preserved. OpenCode's completed plan opened in a separate document for review.",
            )
            ...
            if (!await vscode.workspace.applyEdit(edit)) {
              throw new Error("VS Code did not apply the generated plan to its untitled document")
            }
          }
    ...
    vscode.commands.registerCommand(
      ...
      run(async () => {
        ...
        if (!document || document.languageId !== "markdown") {
          throw new Error("Open the approved Markdown plan before handing it off")
        }
        ...
        const planArtifactRecord = taskArtifacts.list(planSessionID).find((artifact) =>
          artifact.kind === "plan" && artifact.lifecycle === "active"
        )
        if (!planArtifactRecord || planArtifactRecord.kind !== "plan") {
          throw new Error("The approved OpenCode plan artifact is no longer available")
        }
        if (!["approved", "handed-off"].includes(planArtifactRecord.payload.phase)) {
          throw new Error("Approve this saved plan before handing it off")
        }
        if (
        ...
        const choice = await vscode.window.showQuickPick([
          {
            ...
            description: "Implement in the current checkout",
            value: "implementation",
          },
          {
            label: "Isolated worktree session",
            description: "Implement in a new Workbench-owned worktree",
            value: "isolated",
          },
          {
            label: "Multi-run",
            description: `Run the approved plan with 2-${MULTI_RUN_MAX_CANDIDATES} models`,
            value: "multi-run",
          },
        ...
        const recordPlanHandoff = (
          handoff: {
            mode: "implementation" | "isolated" | "multi-run" | "goal"
            sessionIDs?: string[]
            runGroupID?: string
            worktreeID?: string
          },
          ...
          const relatedSessionIDs = [
            ...new Set([...(current.payload.relatedSessionIDs ?? []), ...(handoff.sessionIDs ?? [])]),
          ]
        ...
        const file = {
          type: "file" as const,
          mime: "text/markdown",
          filename: "approved-plan.md",
          url: `data:text/markdown;base64,${Buffer.from(content).toString("base64")}`,
        }
        const implementationPrompt =
          `Implement the approved plan referenced by ${reference.id}. Preserve its acceptance criteria and report validation evidence.`
        if (choice.value === "isolated") {
          ...
          const entry = await startIsolatedTask(client, worktrees, workspacePath, {
            ...
            files: [file],
            admission: {
              prepare: (sessionID, promptID) =>
                contextReceipts.stage(sessionID, promptID, promptFileReceiptItems([file], reference.id), "none"),
              admit: (sessionID, promptID) => {
                contextReceipts.admit(sessionID, promptID)
              },
          ...
          recordPlanHandoff({
            mode: "isolated",
            sessionIDs: entry.sessionID ? [entry.sessionID] : undefined,
            worktreeID: entry.id,
          })
          await stateWriter.flush()
        ...
        if (choice.value === "multi-run") {
          if (!multiRun) throw new Error("OpenCode Multi-run is unavailable")
          requireContinuity()
          const models = controller.chatSnapshot().models.map((model) => ({
            label: model.name,
          ...
          const selected = await vscode.window.showQuickPick(models, {
            canPickMany: true,
            title: "Plan Multi-run",
            placeHolder: `Select 2-${MULTI_RUN_MAX_CANDIDATES} models`,
          })
          if (!selected || selected.length < 2 || selected.length > MULTI_RUN_MAX_CANDIDATES) {
            throw new Error(`Select 2-${MULTI_RUN_MAX_CANDIDATES} models`)
          }
          if (selected.length > MULTI_RUN_DEFAULT_CONCURRENCY) {
            const confirmed = await vscode.window.showWarningMessage(
              `Create ${selected.length} isolated model runs? Up to ${MULTI_RUN_DEFAULT_CONCURRENCY} will use providers concurrently and the remainder will queue.`,
              { modal: true },
              "Create runs",
            )
            if (confirmed !== "Create runs") return
          }
          const repository = await worktrees.repository(workspacePath)
          const baseRef = (await gitRunner.run(["rev-parse", "HEAD"], repository.root)).stdout.trim()
          const mutationID = randomUUID()
          contextReceipts.stage(`run-group:${mutationID}`, mutationID, [{
            id: reference.id,
            kind: "attachment",
            label: "Approved plan",
            contentHash: reference.revision,
            bytes: Buffer.byteLength(content),
          }], "none")
          const receipt = contextReceipts.admit(`run-group:${mutationID}`, mutationID)!
          const group = await multiRun.start({
            mutationID,
            ownerSessionID: planSessionID,
            title: "Implement approved plan",
            repository: repository.root,
            baseRef,
            promptReceiptID: receipt.id,
            prompt: implementationPrompt,
            files: [file],
            runs: selected.map((model, index) => ({ id: `run-${index + 1}`, model: model.value, agent: "build" })),
            worktreeParent: path.join(
              path.dirname(repository.root),
              `.${path.basename(repository.root)}-worktrees`,
              `run-${mutationID.slice(0, 8)}`,
          ...
          recordPlanHandoff({
            mode: "multi-run",
            sessionIDs: group.runs.flatMap((run) => run.session.sessionID === "pending" ? [] : [run.session.sessionID]),
            runGroupID: group.id,
          })
          await stateWriter.flush()
          return
        ...
        contextReceipts.stage(
          ...
          implementationPromptID,
          promptFileReceiptItems([file], reference.id),
          "none",
        )
        try {
        ...
        recordPlanHandoff({
          mode: choice.value === "goal" ? "goal" : "implementation",
          sessionIDs: [implementationSessionID],
        })
        await stateWriter.flush()
        await vscode.commands.executeCommand("opencodeWorkbench.chat.focus")
      }),
    ...
    vscode.commands.registerCommand(
      ...
      run(async () => {
        ...
        const capabilities = detectGitHubSurfaces(
        ...
        const value = await vscode.window.showInputBox({
          title: "GitHub Issue or Pull Request Handoff",
          prompt: "Canonical https://github.com/owner/repository/issues/N or /pull/N URL",
          ignoreFocusOut: true,
          validateInput: (input) => {
            try {
              parseGitHubReference(input)
              return undefined
            } catch (error) {
        ...
        const selectedContext = editor && selectedText
          ? {
        ...
        const github = new NativeGitHubContextService(
          new AuthenticatedGitHubRestProvider({
            getSession: () => vscode.authentication.getSession("github", ["repo"], { createIfNone: true }),
          }),
        )
        const githubContext = await vscode.window.withProgress({
          location: vscode.ProgressLocation.Notification,
          title: `Loading bounded GitHub ${reference.kind} context…`,
          cancellable: false,
        }, () => github.load(reference))
        ...
        const file = {
          type: "file" as const,
          filename: "github-handoff.md",
          mime: "text/markdown",
          url: `data:text/markdown;base64,${Buffer.from(contextDocument).toString("base64")}`,
        }
        const explicitLimits = hasExplicitGitHubContextLimits(githubContext)
        const receiptItems = promptFileReceiptItems(
          [file],
          `github:${reference.owner}/${reference.repository}:${reference.kind}:${reference.number}`,
        ).map((item) => ({ ...item, truncated: explicitLimits || undefined }))
        const receiptCoverage = explicitLimits ? "explicit" as const : "none" as const
        const mode = await vscode.window.showQuickPick([
          {
            label: "Isolated worktree session (Recommended)",
            description: "Implement in a new Workbench-owned branch and checkout",
            value: "isolated",
          },
          {
            label: "Current checkout session",
            description: "Create a normal Workbench session in this checkout",
            value: "current",
          },
        ], {
          title: `Handoff #${reference.number}: ${githubContext.title.text.slice(0, 100)}`,
          placeHolder: "Choose where OpenCode should implement this bounded GitHub snapshot",
        })
        if (!mode) return
        ...
        const nativeActions = [
          ...(reference.kind === "pull-request" && capabilities.openPullRequestChanges
            ? ["Open PR changes in VS Code"]
            : []),
          ...(capabilities.openView ? ["Open GitHub view"] : []),
        ]
        if (nativeActions.length) {
          const open = await vscode.window.showInformationMessage(
            "Bounded GitHub context was handed to OpenCode.",
            ...nativeActions,
          )
          if (open === "Open GitHub view") {
            await vscode.commands.executeCommand("workbench.view.extension.github-pull-requests")
          }
          if (open === "Open PR changes in VS Code") {
            ...
            if (!uri || !await vscode.env.openExternal(vscode.Uri.parse(uri))) {
              throw new Error("The GitHub Pull Requests extension did not open the pull request changes surface")
            }
          }
        }
      }),
    ...
    vscode.commands.registerCommand(
      ...
      run(async (request?: {
        ...
        if (request?.sessionID && !Object.hasOwn(controller.snapshot.sessions, request.sessionID)) {
          throw new Error("The originating OpenCode session is no longer available")
        }
        const originDirectory = originSessionID
          ? controller.snapshot.sessions[originSessionID]?.info.directory
          : undefined
        ...
        const selectedSources = request?.task?.trim() && request.sources?.length
          ...
          : await vscode.window.showQuickPick([
            ...(hasSelection
              ? [{
                label: "Selected editor/debug text",
                description: "Attach only the current explicit selection",
                value: "selection" as const,
              }]
              : []),
            {
              label: "Clipboard console output",
              description: "Read the clipboard once after this explicit choice",
              value: "console" as const,
            },
            {
              label: "Clipboard element metadata",
              description: "Read the clipboard once after this explicit choice",
              value: "element" as const,
            },
            {
              label: "Terminal or task excerpt",
              description: "Read one explicitly chosen excerpt from the clipboard",
              value: "terminal-task" as const,
            },
            {
              label: "Workspace diagnostics summary",
              description: "Attach counts and repository-contained diagnostic paths",
              value: "diagnostics" as const,
            },
            {
              label: "VS Code debug state",
              description: "Attach bounded active-session metadata and repository breakpoints",
              value: "debug" as const,
            },
            {
              label: "Approved page URL",
              description: "Attach one HTTP(S) URL you approve; the page is not fetched",
              value: "url" as const,
            },
            {
              label: "Screenshot file",
              description: "Choose a PNG, JPEG, or WebP explicitly",
              value: "screenshot" as const,
            },
          ], {
            canPickMany: true,
            title: "Attach Browser/Debug Context",
            placeHolder: "No browser proxy or navigation is used",
          })
        if (!selectedSources?.length) return
        if (
          selectedSources.filter((source) =>
            source.value === "console" || source.value === "element" || source.value === "terminal-task"
          ).length > 1
        ) throw new Error("Choose only one clipboard-backed context source")
        const editorSelection = requestedBrowserEditorSelection(
          selectedSources.map((source) => source.value),
          rememberedSelection,
        )
        const task = request?.task?.trim() ||
          await vscode.window.showInputBox({
            title: "Browser-context task",
            prompt: "What should OpenCode do with this context?",
            validateInput: (value) => value.trim() ? undefined : "Enter a task",
          })
        if (!task) return
        const clipboardSource = selectedSources.find((source) =>
          source.value === "console" || source.value === "element" || source.value === "terminal-task"
        )
        ...
        const approvedUrl = selectedSources.some((source) => source.value === "url")
          ? request?.approvedUrl ??
            await vscode.window.showInputBox({
              title: "Approved page URL",
              prompt: "HTTP(S) URL to identify in the context receipt (the page will not be fetched)",
              ignoreFocusOut: true,
            })
          : undefined
        ...
        if (selectedSources.some((source) => source.value === "screenshot")) {
          const selected = await vscode.window.showOpenDialog({
            title: "Choose browser screenshot",
            canSelectMany: false,
            filters: { "Browser screenshots": ["png", "jpg", "jpeg", "webp"] },
          })
          ...
          assertBrowserScreenshotFileBeforeRead({
            ...
            isFile: (screenshotStat.type & vscode.FileType.File) !== 0 &&
              (screenshotStat.type & vscode.FileType.SymbolicLink) === 0,
          })
        ...
        const capture = captureBrowserContext({
          task,
          editorSelection,
          clipboardText: clipboardKind && clipboardText !== undefined
            ? { kind: clipboardKind, text: clipboardText }
            : undefined,
          diagnostics: selectedSources.some((source) => source.value === "diagnostics")
            ? diagnosticsSummary(currentDiagnostics(originDirectory ?? workspacePath))
            : undefined,
          debugState: selectedSources.some((source) => source.value === "debug")
            ? await currentDebugContext(originDirectory ?? workspacePath)
            : undefined,
          approvedUrl,
          screenshot,
        })
        const browserSessionID = originSessionID ??
          await controller.createSession(`Browser context: ${task.slice(0, 100)}`)
        if (controller.snapshot.selectedID !== browserSessionID) {
          throw new Error(
            "The selected OpenCode session changed while browser context was being captured; nothing was sent",
          )
        }
    ...
    vscode.commands.registerCommand(
      ...
      run(async () => {
        ...
        if (!client || !vscode.workspace.workspaceFolders?.length) {
          throw new Error("Open a Git workspace before creating an isolated session")
        }
        const prompt = await vscode.window.showInputBox({
          title: "New Isolated Session",
          prompt: "Initial task (optional)",
          ignoreFocusOut: true,
        })
        ...
        if (prompt.trim()) {
          ...
          try {
            ...
            await worktrees.failDurably(entry.id, {
              code: "INTERNAL",
              message: "OpenCode session creation failed for this isolated worktree",
              retryable: true,
            })
            throw error
          }
    ...
    vscode.commands.registerCommand(
      ...
      run(async () => {
        ...
        const selected = await vscode.window.showQuickPick(
          candidates.map((entry) => ({ label: entry.branch, description: entry.path, entry })),
          { title: "Remove Worktree", placeHolder: "Dirty worktrees are always retained" },
        )
        if (!selected) return
    ...
    vscode.commands.registerCommand(
      ...
      run(async () => {
        const candidates = worktrees.journal().filter((entry) => entry.phase === "removed")
        const selected = await vscode.window.showQuickPick(
          candidates.map((entry) => ({ label: entry.branch, description: entry.repository, entry })),
    ...
    vscode.commands.registerCommand(
      ...
      run(async () => {
        ...
        requireContinuity()
        const choices = controller.chatSnapshot().models.map((model) => ({
          label: model.name,
          description: model.providerID,
          value: `${model.providerID}/${model.id}`,
        }))
        ...
        const group = await multiRun.start({
          mutationID,
          ownerSessionID: controller.snapshot.selectedID,
          title: prompt.trim().slice(0, 160),
          repository: repository.root,
          baseRef,
          promptReceiptID: receipt.id,
          prompt,
          runs: selected.map((choice, index) => ({ id: `run-${index + 1}`, model: choice.value })),
          worktreeParent: path.join(
            path.dirname(repository.root),
            `.${path.basename(repository.root)}-worktrees`,
            `run-${mutationID.slice(0, 8)}`,
        ...
        const active = group.runs.filter((entry) =>
          ["preparing", "admitting", "working", "needs-input"].includes(entry.phase)
        ).length
        void vscode.window.showInformationMessage(
          `Created ${group.runs.length} isolated OpenCode runs; ${active} active and ${
            group.runs.length - active - failed
          } queued${failed ? `; ${failed} failed to start` : ""}.`,
        )
      }),
    ),
    vscode.commands.registerCommand(
    ...
    vscode.commands.registerCommand(
      ...
      run(async () => {
        if (!multiRun) throw new Error("OpenCode Multi-run is unavailable")
        const selected = await vscode.window.showQuickPick(
          runGroups.list().map((group) => ({ label: group.title, description: group.id, group })),
    ...
    vscode.commands.registerCommand(
      ...
      run(async (groupID?: unknown, runID?: unknown) => {
        const runs = runGroups.list().flatMap((group) =>
          group.runs.filter((entry) => !entry.discarded && entry.session.sessionID !== "pending").map((entry) => ({
            label: `${group.title}: ${entry.model}`,
            description: `${entry.phase} · ${entry.session.directory}`,
            group,
            entry,
          }))
        )
        const directRequested = groupID !== undefined || runID !== undefined
        if (directRequested && (typeof groupID !== "string" || typeof runID !== "string")) {
          throw new Error("An exact run group and run ID are required")
        }
        const direct = typeof groupID === "string" && typeof runID === "string"
          ? runs.find((candidate) => candidate.group.id === groupID && candidate.entry.id === runID)
          : undefined
        ...
        if (selected) {
          ...
          await vscode.commands.executeCommand(
            "vscode.openFolder",
            vscode.Uri.file(selected.entry.session.directory),
            true,
          )
        }
      }),
    ...
    vscode.commands.registerCommand(
      ...
      run(async (groupID?: unknown, runID?: unknown) => {
        ...
        const directRun = directGroup && typeof runID === "string"
          ? directGroup.runs.find((entry) => entry.id === runID)
          : undefined
        const direct = directRun && directRun.session.sessionID !== "pending" && !directRun.discarded
          ? { group: directGroup!, entry: directRun }
          : undefined
        if (directRequested && !direct) throw new Error("The requested run is no longer available for diff")
        const picked = direct ?? await vscode.window.showQuickPick(
          runGroups.list().flatMap((group) =>
            group.runs.filter((entry) => entry.session.sessionID !== "pending" && !entry.discarded).map((entry) => ({
              label: `${group.title}: ${entry.model}`,
              description: entry.session.directory,
              group,
              entry,
            }))
          ),
          { title: "Open Run Native Diff" },
        )
        if (!picked) return
        const capture = await diffs.capture({
          repository: picked.entry.session.directory,
          scope: "branch",
          baseRef: picked.group.baseRef,
        })
        if (!capture.snapshot.complete) {
          throw new Error(`Run diff is incomplete: ${capture.snapshot.truncationReason ?? "unknown limitation"}`)
        }
        const names = capture.snapshot.files.map((file) => file.path).slice(0, 500)
        const selected = names.length === 1 ? names[0] : (await vscode.window.showQuickPick(names, {
          title: "Open Run Native Diff",
        ...
        await vscode.commands.executeCommand(
          ...
          original.uri,
          current.uri,
          `${picked.entry.model}: ${content.modifiedPath}`,
        )
      }),
    ),
    vscode.commands.registerCommand(
      "opencodeWorkbench.compareRunResults",
      run(async (groupID?: unknown) => {
        chatProvider.openInEditor("runs")
        const direct = typeof groupID === "string" ? runGroups.get(groupID) : undefined
        const selected = direct ? { group: direct } : await vscode.window.showQuickPick(
          runGroups.list().map((group) => ({ label: group.title, description: `${group.runs.length} runs`, group })),
          { title: "Objective Run Comparison" },
        )
        if (!selected) return
        ...
        const ownerSessionID = selected.group.ownerSessionID ??
          selected.group.runs.find((run) => run.session.sessionID !== "pending")?.session.sessionID
        if (!ownerSessionID) throw new Error("Run comparison has no OpenCode session owner")
        const existing = taskArtifacts.list(ownerSessionID).find((artifact) =>
          artifact.kind === "run-comparison" && artifact.payload.groupID === selected.group.id
        )
        if (existing?.kind === "run-comparison") {
          taskArtifacts.update(ownerSessionID, existing.id, existing.revision, (draft) => {
            if (draft.kind === "run-comparison") draft.payload.rows = rows
          })
        } else {taskArtifacts.create({
            kind: "run-comparison",
            sessionID: ownerSessionID,
            lifecycle: "active",
            payload: { groupID: selected.group.id, rows },
          })}
        await stateWriter.flush()
        void vscode.window.showInformationMessage(
          "Objective comparison refreshed in the Runs workbench. No winner was inferred.",
        )
      }),
    ),
    vscode.commands.registerCommand(
      ...
      run(async (groupID?: unknown) => {
        ...
        chatProvider.openInEditor("runs")
        requireContinuity()
        const candidates = runGroups.list().filter((group) =>
          group.runs.filter((run) => run.phase === "completed" && run.session.sessionID !== "pending" && !run.discarded)
            .length >= 2
        )
        ...
        const selected = direct ? { group: direct } : await vscode.window.showQuickPick(
          candidates.map((group) => ({
            label: group.title,
            description: `${group.runs.length} source runs · ${group.id}`,
            group,
          })),
          { title: "Fusion Source Group" },
        )
        if (!selected) return
        const selectedMode = await vscode.window.showQuickPick([
          {
            label: "Synthesize implementation plan",
            description: "Create a plan without implementing",
            value: "plan" as FusionMode,
            agent: "plan",
          },
          {
            label: "Build combined implementation",
            description: "Implement in a new isolated worktree",
            value: "build" as FusionMode,
            agent: "build",
          },
          {
            label: "Review and choose approach",
            description: "Assess sources without implementing",
            value: "review" as FusionMode,
            agent: "plan",
          },
        ], { title: "Fusion Mode" })
        if (!selectedMode) return
        const availableSources = selected.group.runs.filter((run) =>
          run.phase === "completed" && run.session.sessionID !== "pending" && !run.discarded
        )
        const sourceRuns = availableSources.length <= 5 ? availableSources : await vscode.window.showQuickPick(
          availableSources.map((run) => ({
            label: run.model,
            description: [run.agent, run.variant, run.id].filter(Boolean).join(" · "),
            run,
          })),
        ...
        const comparisonRows = new Map(
          (await runComparison.compare(selected.group, observations)).map((row) => [row.runID, row]),
        )
        const artifacts = await Promise.all(sourceRuns.map(async (run) => {
          const capture = await diffs.capture({
            repository: run.session.directory,
            scope: "branch",
            baseRef: selected.group.baseRef,
          })
          if (!capture.snapshot.complete) {
            throw new Error(
              `Fusion source ${run.id} has an incomplete diff: ${
                capture.snapshot.truncationReason ?? "unknown limitation"
              }`,
            )
          }
          ...
          return {
            ...
            variant: run.variant,
            phase: run.phase,
            unifiedDiff: capture.unifiedDiff,
            diffSnapshot: capture.snapshot,
            evidence: boundedFusionSourceEvidence(observation?.evidence ?? [], run.id, run.session.sessionID),
            objectiveSummary,
            assistantSummary: observation?.assistantSummary,
          }
        }))
    ...
    vscode.commands.registerCommand(
      ...
      run(async (groupID?: unknown, runID?: unknown) => {
        if (!multiRun) throw new Error("OpenCode Multi-run is unavailable")
        const candidates = runGroups.list().flatMap((group) =>
          group.runs.filter((entry) =>
            entry.worktreeID && entry.session.sessionID !== "pending" && !entry.retained && !entry.discarded
          ).map((entry) => ({
            label: `${group.title}: ${entry.model}`,
            description: entry.session.directory,
            group,
        ...
        const direct = typeof groupID === "string" && typeof runID === "string"
          ? candidates.find((candidate) => candidate.group.id === groupID && candidate.entry.id === runID)
          : undefined
        ...
        const selected = direct ??
          await vscode.window.showQuickPick(candidates, {
            title: "Discard Run Safely",
            placeHolder: "Dirty results are retained",
          })
        if (!selected) return
    ...
    vscode.commands.registerCommand(
      ...
      run(async () => {
        ...
        await flushDurableMetadata()
        const selected = await vscode.window.showQuickPick(
          controller.chatSnapshot().models.map((model) => ({
            label: model.name,
            description: model.providerID,
            value: `${model.providerID}/${model.id}`,
          })),
          { title: "Generate Changes Walkthrough" },
        )
        if (!selected) return
        const walkthrough = await walkthroughs.generate(
          capture,
          selected.value,
          async ({ prompt, unifiedDiff, model }) =>
            (await invokeBoundedOpenCode(client!.forDirectory(repository.root), "Changes walkthrough", prompt, model, {
              filename: "changes.diff",
              mime: "text/x-diff",
              url: `data:text/x-diff;base64,${Buffer.from(unifiedDiff).toString("base64")}`,
            })).text,
        )
        ...
        const markdown = [
          `# Changes Walkthrough`,
          ``,
          `Diff: \`${walkthrough.diffHash}\``,
          `Coverage: **${walkthrough.coverage}**`,
          ...(walkthrough.uncoveredFiles?.length ? [`Uncovered: ${walkthrough.uncoveredFiles.join(", ")}`] : []),
          ``,
          ...walkthrough.stops.flatMap((
            stop,
            index,
          ) => [
            `## ${index + 1}. ${stop.title}`,
            ...
            stop.explanation,
            ``,
            ...stop.anchors.map((anchor) => `- ${anchor.file}:${anchor.startLine}-${anchor.endLine} (${anchor.side})`),
            ``,
          ]),
        ].join("\n")
    ...
    vscode.commands.registerCommand(
      ...
      run(async (documentID?: string, stopID?: string) => {
        ...
        const selectedDocument = documentID
          ...
          : await vscode.window.showQuickPick(
            documents.map((document) => ({
              label: new Date(document.generatedAt).toLocaleString(),
              description: `${document.coverage} · ${document.model}`,
              document,
            })),
            { title: "Open Walkthrough Stop" },
          )
        if (!selectedDocument) return
        const selectedStop = stopID
          ? (() => {
            const stop = selectedDocument.document.stops.find((candidate) => candidate.id === stopID)
            return stop ? { stop } : undefined
          })()
          : await vscode.window.showQuickPick(
            selectedDocument.document.stops.map((stop) => ({ label: stop.title, description: stop.importance, stop })),
            { title: "Walkthrough Stop" },
          )
        ...
        if (
          !capture.snapshot.complete || capture.snapshot.unifiedDiffHash !== selectedDocument.document.diffHash
        ) throw new Error("Walkthrough is stale; regenerate it for the current complete diff")
        await openDiffAnchor(diffs, capture, anchor, `${anchor.file} (Walkthrough)`)
      }),
    ),
    vscode.commands.registerCommand(
      "opencodeWorkbench.reviewChanges",
      run(async (directory?: unknown, baseRef?: unknown, requestedOwnerSessionID?: unknown) => {
        if (!client || !controller) throw new Error("OpenCode is unavailable")
        ...
        if (requestedOwnerSessionID !== undefined && typeof requestedOwnerSessionID !== "string") {
          throw new Error("Invalid review owner session")
        }
        const explicitOwner = typeof requestedOwnerSessionID === "string"
          ? controller.snapshot.sessions[requestedOwnerSessionID]
          : undefined
        if (typeof requestedOwnerSessionID === "string" && !explicitOwner) {
          throw new Error("The requested OpenCode review owner is no longer available")
        }
        const explicitOwnerDirectory = explicitOwner
          ? await fs.realpath(explicitOwner.info.directory).catch(() => undefined)
          : undefined
        if (explicitOwner && explicitOwnerDirectory !== repository.root) {
          throw new Error("The requested OpenCode review owner belongs to a different repository")
        }
        const originatingDirectory = originatingSession?.directory
          ? await fs.realpath(originatingSession.directory).catch(() => undefined)
          : undefined
        const ownerSessionID = explicitOwner?.info.id ??
          (originatingDirectory === repository.root ? originatingSession?.id : undefined)
        ...
        const review = await reviews.generate(capture, selected.value, async ({ prompt, unifiedDiff, model }) => {
          producer = await invokeBoundedOpenCode(client!.forDirectory(repository.root), "Code review", prompt, model, {
            filename: "changes.diff",
            mime: "text/x-diff",
            url: `data:text/x-diff;base64,${Buffer.from(unifiedDiff).toString("base64")}`,
          })
        ...
        taskArtifacts.create({
          ...
          payload: {
            document: review,
            repository: repository.root,
            baseRef: capture.snapshot.baseRef,
        ...
        const markdown = [
          `# OpenCode Review Findings`,
          ``,
          `> These are model findings, not deterministic facts. Diff: \`${review.diffHash}\``,
          ``,
          ...(review.findings.length
            ? review.findings.flatMap((
              finding,
    ...
    vscode.commands.registerCommand(
      "opencodeWorkbench.openReviewFinding",
      run(async (sessionID?: unknown, artifactID?: unknown, findingID?: unknown) => {
        ...
        const direct = typeof sessionID === "string" && typeof artifactID === "string"
          ...
          : undefined
        const selectedArtifact = direct ??
          (await vscode.window.showQuickPick(
            reviews.map((artifact) => ({
              label: new Date(artifact.payload.document.generatedAt).toLocaleString(),
              description: `${artifact.payload.document.findings.length} findings · ${artifact.payload.document.model}`,
              artifact,
            })),
            { title: "Open Review Finding" },
          ))?.artifact
        if (!selectedArtifact) return
        const directFinding = typeof findingID === "string"
          ? selectedArtifact.payload.document.findings.find((finding) => finding.id === findingID)
          : undefined
        const selectedFinding = directFinding ??
          (await vscode.window.showQuickPick(
            selectedArtifact.payload.document.findings.map((finding) => ({
        ...
        if (!anchor) return
        const current = await diffs.capture({
          repository: selectedArtifact.payload.repository,
          scope: selectedArtifact.payload.scope,
          baseRef: selectedArtifact.payload.baseRef,
        })
        if (
          !current.snapshot.complete || current.snapshot.unifiedDiffHash !== selectedArtifact.payload.diffHash ||
          selectedArtifact.payload.stale
        ) {
          if (!selectedArtifact.payload.stale) {
            taskArtifacts.update(
    ...
    vscode.commands.registerCommand(
      ...
      run(async () => {
        ...
        const tasks = await Promise.all(
          (await vscode.tasks.fetchTasks()).map(async (task) => ({ task, repository: await taskRepository(task) })),
        )
        ...
        const selected = await vscode.window.showQuickPick(
          available.map(({ task }) => ({
            label: task.name,
            description: [task.source, task.group?.id].filter(Boolean).join(" · "),
            task,
          })),
          { title: "Run Task and Capture Evidence" },
        )
        if (!selected) return
        ...
        evidence.record({
          kind: testTask ? "test" : "task",
          label: selected.task.name,
          status: exitCode === 0 ? "passed" : exitCode === undefined ? "unknown" : "failed",
          sourceID: selected.task.definition.type,
          ...evidenceScope,
          summary: exitCode === undefined
            ? "Task ended without a reported exit code"
            : `Task exited with code ${exitCode}`,
        ...
        evidence.record({
          kind: "diagnostics",
          label: "VS Code diagnostics delta",
          ...diagnosticEvidence,
          ...evidenceScope,
        })
        await flushDurableMetadata()
        const document = await vscode.workspace.openTextDocument({
          language: "json",
          content: JSON.stringify(evidence.list(evidenceScope), null, 2),
        })
        await vscode.window.showTextDocument(document, { preview: false })
      }),
    ),
    vscode.commands.registerCommand(
      ...
      run(async () => {
        ...
        const latestAssistantResult = session.messages.slice().reverse().find((message) =>
          message.info.role === "assistant"
        )?.parts.filter((part) => part.type === "text" && part.text).map((part) => part.text).join("\n")
        ...
        const verificationInput = {
          ...
          diffSummary: turnDiff
            ? `${turnDiff.snapshot.files.length} files · ${turnDiff.snapshot.unifiedDiffHash} · ${
              turnDiff.snapshot.complete ? "complete" : `incomplete: ${turnDiff.snapshot.truncationReason}`
            }`
            : undefined,
          ...
          remainingLimits: {
            tokens: goal.remainingTokens,
            autoTurns: goal.maxAutoTurns === undefined
              ? undefined
              : Math.max(0, goal.maxAutoTurns - (goal.autoTurns ?? 0)),
            durationSeconds: goal.maxDurationSeconds === undefined
              ? undefined
              : Math.max(0, goal.maxDurationSeconds - (goal.timeUsedSeconds ?? 0)),
          },
        ...
        const verification = await vscode.window.withProgress(
          ...
          async (_progress, cancellation) => {
            ...
            const subscription = cancellation.onCancellationRequested(() =>
              abort.abort(new Error("Goal verification cancelled"))
            )
            try {
        ...
        const verifierEvidence = evidence.record({
          ...
          status: verdict.verdict === "complete"
            ? "passed"
            : verdict.verdict === "continue" || verdict.verdict === "blocked"
            ? "warning"
            : "unknown",
        ...
        const evidenceReferences = [
          ...new Set([
            ...
            ...scopedEvidence.map((entry) => entry.id),
            verifierEvidence.id,
          ]),
        ]
        const currentGoal = controller.chatSnapshot().session?.id === session.id
          ? controller.chatSnapshot().session?.goal
          : undefined
        const fresh = currentGoal?.settlementGeneration === goal.settlementGeneration
        if (!finalAttempt?.sessionID) {
          throw new Error("OpenCode verifier did not return its producer session provenance")
        }
        ...
        const choice = fresh
          ? await vscode.window.showInformationMessage(
            `Verifier verdict: ${verdict.verdict} (${verdict.confidence} confidence). Apply it to this goal?`,
            { modal: true, detail: verdict.reason },
            "Apply verdict",
          )
          : undefined
        let applied = false
        if (choice === "Apply verdict") {
          if (
            controller.chatSnapshot().session?.id !== session.id ||
            controller.chatSnapshot().session?.goal?.settlementGeneration !== goal.settlementGeneration
          ) {
            throw new Error(
              "The goal changed while verification was running; rerun verification before applying this verdict",
            )
          }
          await controller.send(
            `/goal verdict ${
              JSON.stringify({
                ...
                missing_criteria: verdict.missingCriteria,
                confidence: verdict.confidence,
                evidence_references: evidenceReferences,
                expected_generation: goal.settlementGeneration,
              })
            }`,
          )
          applied = true
    ...
    vscode.commands.registerCommand("opencodeWorkbench.setServerPassword", async () => {
      ...
      const password = await vscode.window.showInputBox({
        ...
        password: true,
        prompt: "Leave empty to remove the stored password",
      })
    ...
    vscode.commands.registerCommand(
      ...
      run(async () => {
        ...
        const executable =
          vscode.workspace.getConfiguration("opencodeWorkbench").get<string>("executablePath")?.trim() || "opencode"
  ...
  const participant = vscode.chat.createChatParticipant(
    ...
    async (request, _chatContext, stream, token) => {
      ...
      const sessionID = controller.chatSnapshot().session?.id ??
        await controller.createSession(`Chat handoff: ${prompt.slice(0, 100)}`)
      const promptID = createOpenCodeMessageID()
      contextReceipts.stage(sessionID, promptID, [], "none")
      try {
      ...
      stream.markdown(
        "Handed this task to the selected OpenCode session. The actual model loop runs in OpenCode, not in this chat participant.",
      )
  ...
  context.subscriptions.push(vscode.workspace.onDidChangeConfiguration((event) => {
    if (
      event.affectsConfiguration("opencodeWorkbench.serverMode") ||
      event.affectsConfiguration("opencodeWorkbench.executablePath") ||
      event.affectsConfiguration("opencodeWorkbench.managedServerStartupTimeout")
    ) {
    ...
    if (
      serverMode() !== "external" ||
      (!event.affectsConfiguration("opencodeWorkbench.serverUrl") &&
        !event.affectsConfiguration("opencodeWorkbench.serverUsername") &&
        !event.affectsConfiguration("opencodeWorkbench.serverEnvironmentFile"))
    ) return
...
export async function deactivate(): Promise<void> {
  ...
  const settle = async (operations: Array<Promise<unknown> | undefined>): Promise<void> => {
    const results = await Promise.allSettled(
      operations.filter((operation): operation is Promise<unknown> => operation !== undefined),
    )
