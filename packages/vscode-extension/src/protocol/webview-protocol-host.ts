...
export interface ProtocolObservation {
  type:
    ...
    | "protocol.request.cancelled"
    | "protocol.resync.recovered"
    | "protocol.event.published"
    | "protocol.snapshot.sent"
    | "protocol.epoch.rotated"
  requestID?: string
  mutationID?: string
  revision?: number
...
function requestID(value: unknown): string {
  return record(value) && typeof value.id === "string" && value.id ? value.id.slice(0, 256) : "invalid"
}
...
function requestCategory(value: unknown): string {
  return value === "workbench.dispatch"
    ? "dispatch"
    : value === "protocol.resync"
    ? "resync"
    : value === "protocol.cancel"
    ? "cancel"
    : "unknown"
}
...
function responseError(
  ...
  const failure = error instanceof ProtocolValidationError ? error : new ProtocolValidationError(
...
export class WebviewProtocolHost<TState, TInbound, TOutbound> {
  ...
  async receive(surfaceID: string, input: unknown): Promise<boolean> {
    ...
    const observation = {
      ...
      revision: Number.isSafeInteger(request?.expectedRevision) && Number(request?.expectedRevision) >= 0
    ...
    if (category === "cancel") {
      ...
      this.observe({
        type: "protocol.request.cancelled",
        requestID: targetRequestID,
        durationMilliseconds: Date.now() - startedAt,
        transition: cancelled ? "cancelled" : "not-found",
      })
    }
    if (category === "resync" && response.ok && record(response.result)) {
      const recovery = response.result.kind === "snapshot"
        ? "snapshot"
        : response.result.kind === "replay"
        ? "replay"
        : "unknown"
      const revision = recovery === "snapshot" && record(response.result.snapshot) &&
          Number.isSafeInteger(response.result.snapshot.revision)
        ? Number(response.result.snapshot.revision)
        : undefined
      this.observe({
        type: "protocol.resync.recovered",
  ...
  async publishTo(surfaceID: string, message: TOutbound): Promise<void> {
    ...
    this.observe({
      type: "protocol.event.published",
      revision: event.nextRevision,
      transition: surface.visible ? "visible" : "queued-hidden",
    })
  ...
  private async recovery(
    ...
    return replay.kind === "snapshot" ? replay : {
      kind: "replay",
