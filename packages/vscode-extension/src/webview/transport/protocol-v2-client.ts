...
import { type WebviewApi, WebviewTransportClient } from "./client.js"
...
export class WorkbenchProtocolClient<TOutbound, TInbound, TState = unknown> {
  ...
  private dispatch(message: TOutbound): Promise<unknown> {
    ...
    const request: Request<"workbench.dispatch", TOutbound> = {
      ...
      ...(record(message) && typeof message.sessionID === "string" ? { sessionID: message.sessionID } : {}),
  ...
  private applyEvent(event: ReturnType<typeof parseEvent>): void {
    if (event.type === "workbench.snapshot") {
      ...
      if (
        !record(snapshot) || typeof snapshot.epoch !== "string" ||
    ...
    if (message === undefined) {
      const type = record(payload.message) && typeof payload.message.type === "string"
        ? payload.message.type
        : "unknown"
      const keys = record(payload.message)
        ? Object.keys(payload.message).slice(0, 20).sort().join(",")
  ...
  private drainRecoveryEvents(): void {
    ...
    for (let index = 0; index < buffered.length; index += 1) {
      ...
      const position = this.cursor.position()
      const payload = record(event.payload) ? event.payload as unknown as WorkbenchEventPayload : undefined
      if (
