...
export function goalFormMarkup(draft: GoalFormDraft, options: GoalFormRenderOptions = {}): string {
  ...
  return `<form id="goal-configuration-form" class="goal-form" data-goal-form novalidate><header><h2>Keep working until done</h2><p>Give OpenCode a clear outcome and independently checkable completion conditions. Exact execution controls stay under Advanced settings.</p></header>${summary}<label for="goal-objective">Objective</label><textarea id="goal-objective" name="objective" rows="3" maxlength="${GOAL_FORM_LIMITS.objectiveCharacters}"${errorAttributes(objectiveError, "goal-objective-error")}>${escapeHtml(draft.objective)}</textarea>${inlineError(objectiveError, "goal-objective-error")}<fieldset class="goal-criteria"><legend>Done when</legend><p>Describe outcomes that can be checked, rather than implementation steps.</p>${inlineError(criteriaError, "goal-criteria-error")}<ol>${criteria}</ol><button type="button" data-goal-criterion-action="add"${draft.criteria.length >= GOAL_FORM_LIMITS.criteria ? " disabled" : ""}>Add condition</button></fieldset><fieldset class="goal-presets"><legend>Effort</legend><button type="button" data-goal-preset="quick">Short</button><button type="button" data-goal-preset="bounded">Standard</button><button type="button" data-goal-preset="thorough">Extended</button></fieldset><label class="goal-verifier-toggle"><input type="checkbox" name="verifierEnabled"${draft.verifierEnabled ? " checked" : ""}> Verify before marking the work complete</label><details class="goal-advanced"><summary>Advanced settings</summary><fieldset class="goal-limits"><legend>Exact execution limits</legend>${limitMarkup("tokenBudget", "Token budget", "tokens", draft.tokenBudget, errors)}${limitMarkup("maxAutoTurns", "Auto-turn limit", "turns", draft.maxAutoTurns, errors)}${limitMarkup("maxDurationSeconds", "Duration limit", "seconds", draft.maxDurationSeconds, errors)}${unlimited ? `<p class="goal-unlimited-warning" role="status">${unlimited === 3 ? "All execution limits are unlimited." : `${unlimited} execution ${unlimited === 1 ? "limit is" : "limits are"} unlimited.`} The goal continues until another limit, a user action, or OpenCode settlement stops it.</p>` : ""}</fieldset><fieldset class="goal-verifier"><legend>Verification details</legend><label for="goal-verifier-model">Verifier model</label><select id="goal-verifier-model" name="verifierModel"${errorAttributes(modelError, "goal-verifier-model-error")}>${optionMarkup(options.models ?? [], draft.verifierModel, "Default model")}</select>${inlineError(modelError, "goal-verifier-model-error")}<label for="goal-verifier-agent">Verifier agent</label><select id="goal-verifier-agent" name="verifierAgent"${errorAttributes(agentError, "goal-verifier-agent-error")}>${optionMarkup(options.agents ?? [], draft.verifierAgent, "Plan agent (default)")}</select>${inlineError(agentError, "goal-verifier-agent-error")}<label for="goal-verifier-timeout">Verifier timeout (milliseconds)</label><input id="goal-verifier-timeout" name="verifierTimeoutMilliseconds" type="number" inputmode="numeric" min="${GOAL_FORM_LIMITS.verifierTimeoutMinimum}" max="${GOAL_FORM_LIMITS.verifierTimeoutMaximum}" value="${escapeHtml(draft.verifierTimeoutMilliseconds)}"${errorAttributes(timeoutError, "goal-verifier-timeout-error")}>${inlineError(timeoutError, "goal-verifier-timeout-error")}<label for="goal-block-threshold">Repeated blocked-verdict threshold</label><input id="goal-block-threshold" name="repeatedBlockThreshold" type="number" inputmode="numeric" min="${GOAL_FORM_LIMITS.repeatedBlockMinimum}" max="${GOAL_FORM_LIMITS.repeatedBlockMaximum}" value="${escapeHtml(draft.repeatedBlockThreshold)}"${errorAttributes(thresholdError, "goal-block-threshold-error")}>${inlineError(thresholdError, "goal-block-threshold-error")}</fieldset></details><input type="hidden" name="expectedSettlementGeneration" value="${draft.expectedSettlementGeneration}"><footer class="goal-form-actions"><button type="button" data-goal-form-action="reset">Reset</button><button type="button" data-goal-form-action="verify">Verify now</button><button type="submit" class="primary-action"${validation.valid ? "" : " aria-describedby=\"goal-form-validation-note\""}>Update goal</button><span id="goal-form-validation-note" class="visually-hidden">The extension host validates all fields and the current goal generation again before sending this configuration to OpenCode.</span></footer></form>`
}
