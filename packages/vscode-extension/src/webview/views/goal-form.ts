...
const PRESETS: Record<GoalFormPreset, {
  ...
  quick: { tokenBudget: 20_000, maxAutoTurns: 4, maxDurationSeconds: 900, verifierTimeoutMilliseconds: 45_000, repeatedBlockThreshold: 2 },
  bounded: { tokenBudget: 100_000, maxAutoTurns: 12, maxDurationSeconds: 3_600, verifierTimeoutMilliseconds: 60_000, repeatedBlockThreshold: 3 },
  thorough: { tokenBudget: 250_000, maxAutoTurns: 30, maxDurationSeconds: 10_800, verifierTimeoutMilliseconds: 120_000, repeatedBlockThreshold: 3 },
}
...
export function addGoalCriterion(draft: GoalFormDraft, value = ""): GoalFormDraft {
  ...
  return { ...draft, criteria: [...draft.criteria.map((criterion) => ({ ...criterion })), { id: `criterion-${index}`, value }] }
}

export function removeGoalCriterion(draft: GoalFormDraft, id: string): GoalFormDraft {
  if (draft.criteria.length <= 1) return draft
  return { ...draft, criteria: draft.criteria.filter((criterion) => criterion.id !== id).map((criterion) => ({ ...criterion })) }
}
...
function parsePositiveInteger(
  ...
  if (!Number.isSafeInteger(parsed) || parsed < minimum || parsed > maximum) {
    errors.push({ field, message: `${label} must be between ${minimum.toLocaleString()} and ${maximum.toLocaleString()}.` })
    return undefined
  }
  return parsed
}

function parseLimit(field: string, label: string, draft: GoalLimitDraft, errors: GoalFormError[]): number | null | undefined {
...
function normalizedIdentifier(field: string, label: string, value: string, errors: GoalFormError[]): string | null {
  ...
  if (characters(normalized) > GOAL_FORM_LIMITS.verifierIdentifierCharacters) {
    errors.push({ field, message: `${label} must be at most ${GOAL_FORM_LIMITS.verifierIdentifierCharacters.toLocaleString()} characters.` })
    return null
  }
  return normalized
}
...
function configuration(draft: GoalFormDraft): { payload?: GoalConfigurationPayload; errors: GoalFormError[] } {
  ...
  if (!objective) errors.push({ field: "objective", message: "Enter a goal objective." })
    ...
    errors.push({ field: "objective", message: `Goal objective must be at most ${GOAL_FORM_LIMITS.objectiveCharacters.toLocaleString()} characters.` })
  }

  if (!draft.criteria.length) errors.push({ field: "criteria", message: "Add at least one independently checkable acceptance criterion." })
  if (draft.criteria.length > GOAL_FORM_LIMITS.criteria) errors.push({ field: "criteria", message: `Use at most ${GOAL_FORM_LIMITS.criteria} acceptance criteria.` })
  const ids = new Set<string>()
  const acceptanceCriteria = draft.criteria.map((criterion, index) => {
    const field = `criterion:${criterion.id || index}`
    if (!criterion.id || ids.has(criterion.id)) errors.push({ field, message: `Acceptance criterion ${index + 1} has an invalid row identity.` })
    ...
    if (!normalized) errors.push({ field, message: `Acceptance criterion ${index + 1} must not be empty.` })
      ...
      errors.push({ field, message: `Acceptance criterion ${index + 1} must be at most ${GOAL_FORM_LIMITS.criterionCharacters.toLocaleString()} characters.` })
    }
    return normalized
  })
  ...
  if (!Number.isSafeInteger(draft.expectedSettlementGeneration) || draft.expectedSettlementGeneration < 0) {
    errors.push({ field: "expectedSettlementGeneration", message: "Goal settlement generation is invalid; refresh the goal before saving." })
  }

  if (errors.length || tokenBudget === undefined || maxAutoTurns === undefined || maxDurationSeconds === undefined ||
    verifierTimeoutMilliseconds === undefined || repeatedBlockThreshold === undefined) return { errors }
  return {
    errors,
    payload: {
...
function optionMarkup(options: readonly GoalFormOption[], current: string, emptyLabel: string): string {
  ...
  const retained = current && !known.has(current)
    ...
    : ""
  return `<option value=""${selected("", current)}>${escapeHtml(emptyLabel)}</option>${retained}${options.map((option) => {
    const detail = option.description ? ` · ${option.description}` : ""
    return `<option value="${escapeHtml(option.value)}"${selected(option.value, current)}>${escapeHtml(`${option.label}${detail}`)}</option>`
  }).join("")}`
}
...
function limitMarkup(
  ...
  return `<div class="goal-limit" data-goal-limit="${key}"><label for="goal-${key}">${escapeHtml(label)}</label><div class="goal-limit-controls"><input id="goal-${key}" name="${key}" type="number" inputmode="numeric" min="1" max="${GOAL_FORM_LIMITS.numericMaximum}" value="${escapeHtml(value.value)}"${value.unlimited ? " disabled" : ""}${errorAttributes(error, errorID)}><span>${escapeHtml(unit)}</span><label class="goal-unlimited"><input type="checkbox" data-goal-unlimited="${key}"${value.unlimited ? " checked" : ""}> Unlimited</label></div>${inlineError(error, errorID)}</div>`
}
...
export function goalFormMarkup(draft: GoalFormDraft, options: GoalFormRenderOptions = {}): string {
  ...
  const unlimited = [draft.tokenBudget, draft.maxAutoTurns, draft.maxDurationSeconds].filter((limit) => limit.unlimited).length
  const summary = errors.length
    ? `<section class="goal-form-errors" role="alert" aria-live="polite"><strong>Fix ${errors.length} goal configuration ${errors.length === 1 ? "issue" : "issues"}</strong><ul>${errors.map((error) => `<li>${escapeHtml(error.message)}</li>`).join("")}</ul></section>`
    : ""
  const criteria = draft.criteria.map((criterion, index) => {
    ...
    return `<li class="goal-criterion" data-goal-criterion-id="${escapeHtml(criterion.id)}"><label for="goal-criterion-${index}">Criterion ${index + 1}</label><textarea id="goal-criterion-${index}" name="acceptanceCriteria" rows="2" maxlength="${GOAL_FORM_LIMITS.criterionCharacters}"${errorAttributes(error, errorID)}>${escapeHtml(criterion.value)}</textarea><div class="goal-criterion-actions"><button type="button" data-goal-criterion-action="up" aria-label="Move criterion ${index + 1} up"${index === 0 ? " disabled" : ""}>Up</button><button type="button" data-goal-criterion-action="down" aria-label="Move criterion ${index + 1} down"${index === draft.criteria.length - 1 ? " disabled" : ""}>Down</button><button type="button" data-goal-criterion-action="remove" aria-label="Remove criterion ${index + 1}"${draft.criteria.length <= 1 ? " disabled" : ""}>Remove</button></div>${inlineError(error, errorID)}</li>`
  }).join("")
  ...
  return `<form id="goal-configuration-form" class="goal-form" data-goal-form novalidate><header><h2>Goal configuration</h2><p>OpenCode owns goal execution. Workbench validates this form before asking the OpenCode companion to apply it.</p></header>${summary}<fieldset class="goal-presets"><legend>Starting point</legend><button type="button" data-goal-preset="quick">Quick</button><button type="button" data-goal-preset="bounded">Bounded</button><button type="button" data-goal-preset="thorough">Thorough</button></fieldset><label for="goal-objective">Objective</label><textarea id="goal-objective" name="objective" rows="3" maxlength="${GOAL_FORM_LIMITS.objectiveCharacters}"${errorAttributes(objectiveError, "goal-objective-error")}>${escapeHtml(draft.objective)}</textarea>${inlineError(objectiveError, "goal-objective-error")}<fieldset class="goal-criteria"><legend>Acceptance criteria</legend><p>Use independently checkable outcomes rather than implementation steps.</p>${inlineError(criteriaError, "goal-criteria-error")}<ol>${criteria}</ol><button type="button" data-goal-criterion-action="add"${draft.criteria.length >= GOAL_FORM_LIMITS.criteria ? " disabled" : ""}>Add criterion</button></fieldset><fieldset class="goal-limits"><legend>Execution limits</legend>${limitMarkup("tokenBudget", "Token budget", "tokens", draft.tokenBudget, errors)}${limitMarkup("maxAutoTurns", "Auto-turn limit", "turns", draft.maxAutoTurns, errors)}${limitMarkup("maxDurationSeconds", "Duration limit", "seconds", draft.maxDurationSeconds, errors)}${unlimited ? `<p class="goal-unlimited-warning" role="status">${unlimited === 3 ? "All execution limits are unlimited." : `${unlimited} execution ${unlimited === 1 ? "limit is" : "limits are"} unlimited.`} The goal continues until another limit, a user action, or OpenCode settlement stops it.</p>` : ""}</fieldset><fieldset class="goal-verifier"><legend>Independent verifier</legend><label class="goal-verifier-toggle"><input type="checkbox" name="verifierEnabled"${draft.verifierEnabled ? " checked" : ""}> Enable independent verification</label><label for="goal-verifier-model">Verifier model</label><select id="goal-verifier-model" name="verifierModel"${errorAttributes(modelError, "goal-verifier-model-error")}>${optionMarkup(options.models ?? [], draft.verifierModel, "Default model")}</select>${inlineError(modelError, "goal-verifier-model-error")}<label for="goal-verifier-agent">Verifier agent</label><select id="goal-verifier-agent" name="verifierAgent"${errorAttributes(agentError, "goal-verifier-agent-error")}>${optionMarkup(options.agents ?? [], draft.verifierAgent, "Plan agent (default)")}</select>${inlineError(agentError, "goal-verifier-agent-error")}<label for="goal-verifier-timeout">Verifier timeout (milliseconds)</label><input id="goal-verifier-timeout" name="verifierTimeoutMilliseconds" type="number" inputmode="numeric" min="${GOAL_FORM_LIMITS.verifierTimeoutMinimum}" max="${GOAL_FORM_LIMITS.verifierTimeoutMaximum}" value="${escapeHtml(draft.verifierTimeoutMilliseconds)}"${errorAttributes(timeoutError, "goal-verifier-timeout-error")}>${inlineError(timeoutError, "goal-verifier-timeout-error")}<label for="goal-block-threshold">Repeated blocked-verdict threshold</label><input id="goal-block-threshold" name="repeatedBlockThreshold" type="number" inputmode="numeric" min="${GOAL_FORM_LIMITS.repeatedBlockMinimum}" max="${GOAL_FORM_LIMITS.repeatedBlockMaximum}" value="${escapeHtml(draft.repeatedBlockThreshold)}"${errorAttributes(thresholdError, "goal-block-threshold-error")}>${inlineError(thresholdError, "goal-block-threshold-error")}</fieldset><input type="hidden" name="expectedSettlementGeneration" value="${draft.expectedSettlementGeneration}"><footer class="goal-form-actions"><button type="button" data-goal-form-action="reset">Reset</button><button type="button" data-goal-form-action="verify">Verify now</button><button type="submit" class="primary-action"${validation.valid ? "" : " aria-describedby=\"goal-form-validation-note\""}>Save configuration</button><span id="goal-form-validation-note" class="visually-hidden">The extension host validates all fields and the current goal generation again before sending this configuration to OpenCode.</span></footer></form>`
}
