...
function jobsMarkup(snapshot: ChatSnapshot): string {
  ...
  const filters = `<div class="inspector-actions" role="search" aria-label="Job filters"><label>Search <input type="search" data-inspector-key="job-filter:text" data-job-filter="text" autocomplete="off" placeholder="Title, model, phase, or command"></label><label>Kind <select data-inspector-key="job-filter:kind" data-job-filter="kind"><option value="">All kinds</option><option value="delegation">Delegations</option><option value="run">Runs</option><option value="session">Child sessions</option><option value="worktree">Worktrees</option><option value="terminal">Terminals</option></select></label><label>Session ID <input type="search" data-inspector-key="job-filter:session" data-job-filter="session" autocomplete="off" spellcheck="false" placeholder="Exact or partial ID"></label><label>Run ID <input type="search" data-inspector-key="job-filter:run" data-job-filter="run" autocomplete="off" spellcheck="false" placeholder="Exact or partial ID"></label></div><p class="placeholder" role="status" aria-live="polite" data-job-filter-status>Showing ${[...groups.values()].reduce((total, rows) => total + rows.length, 0)} jobs.</p>`
  return `<h2>Jobs</h2>${projectionNotice(snapshot, ["lineage", "worktrees", "ptys"])}${filters}${canBackground ? `<div class="inspector-actions"><button type="button" data-job-session="${escapeHtml(snapshot.session!.id)}" data-job-action="background">Background OpenCode child sessions</button></div>` : ""}${["Needs input", "Running", "Failed", "Completed"].map((group) => {
    ...
    return rows.length ? `<section data-job-group><h3>${group} <span data-job-group-count>${rows.length}</span></h3><ul class="inspector-list">${rows.join("")}</ul></section>` : ""
  }).join("")}`
}
...
function healthMarkup(snapshot: ChatSnapshot, formatDate: (value: number) => string): string {
  ...
  const recent = snapshot.trace?.slice(-20).reverse() ?? []
  return `<h2>Health</h2><dl class="inspector-metrics"><dt>OpenCode</dt><dd>${escapeHtml(health.openCodeVersion ?? "Unknown")}</dd><dt>Server</dt><dd>${escapeHtml(health.serverState)}</dd><dt>Companion</dt><dd>${escapeHtml(health.pluginState)}</dd><dt>Event stream</dt><dd>${escapeHtml(health.eventStream.state)}</dd><dt>Last event</dt><dd>${escapeHtml(lastEvent)}</dd><dt>Queue</dt><dd>${health.requestQueueDepth}</dd></dl><div class="inspector-actions"><button type="button" data-health-action="refresh">Refresh</button><button type="button" data-health-action="reconnect">Reconnect</button><button type="button" data-health-action="logs">Logs</button><button type="button" data-health-action="trace">Raw trace</button></div><h2>Recent sanitized events</h2>${recent.length ? `<ul class="inspector-list">${recent.map((entry) => `<li><strong>${escapeHtml(entry.type)}</strong><small>${escapeHtml(formatDate(entry.timestamp))}${entry.transition ? ` · ${escapeHtml(entry.transition)}` : ""}</small></li>`).join("")}</ul>` : `<p class="placeholder">No trace events recorded.</p>`}`
}
...
function contextMarkup(snapshot: ChatSnapshot, formatDate: (value: number) => string): string {
  ...
  const usage = context
    ? `<div class="context-budget"><div class="context-ring" role="meter" aria-label="OpenCode context usage" aria-valuemin="0" aria-valuemax="100"${context.usagePercent === undefined ? "" : ` aria-valuenow="${Math.round(context.usagePercent)}"`} style="--context-used:${context.usagePercent ?? 0}"><span>${context.usagePercent === undefined ? "?" : `${Math.round(context.usagePercent)}%`}</span></div><dl class="inspector-metrics"><dt>Model</dt><dd>${escapeHtml(context.model ?? "Unknown")}</dd><dt>Actual total</dt><dd>${context.totalTokens.toLocaleString()} tokens</dd><dt>Limit</dt><dd>${context.contextLimit === undefined ? "Unknown" : context.contextLimit.toLocaleString()}</dd><dt>Input</dt><dd>${context.inputTokens.toLocaleString()}</dd><dt>Output</dt><dd>${context.outputTokens.toLocaleString()}</dd><dt>Reasoning</dt><dd>${context.reasoningTokens.toLocaleString()}</dd><dt>Cache read</dt><dd>${context.cacheReadTokens.toLocaleString()}</dd><dt>Cache write</dt><dd>${context.cacheWriteTokens.toLocaleString()}</dd><dt>Cost</dt><dd>$${context.cost.toFixed(4)}</dd></dl></div><p class="placeholder"><strong>Estimated receipt tokens:</strong> ${receiptEstimate.toLocaleString()}. This estimate is separate from OpenCode’s actual usage.</p>`
