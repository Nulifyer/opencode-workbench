...
Deno.test("browser capture includes only explicitly supplied bounded context", () => {
  ...
  assertEquals(result.receiptItems.map((item) => item.kind), [
    "selection",
    "debug",
    "diagnostics",
    "debug",
...
Deno.test("browser capture rejects empty and oversized implicit data", () => {
  ...
  assertThrows(() =>
    captureBrowserContext({
      task: "Diagnose",
      screenshot: { name: "page.exe", mime: "image/png", bytes: new Uint8Array([1]) },
    })
  )
  assertThrows(
    () => captureBrowserContext({ task: "Diagnose", approvedUrl: "https://user:secret@example.com/" }),
    Error,
...
Deno.test("browser capture sends an approved URL but strips query secrets from durable receipt metadata", () => {
  const result = captureBrowserContext({
    task: "Trace callback",
    approvedUrl: "https://example.test/callback?access_token=do-not-store#credential",
  })
...
Deno.test("browser screenshot stat is bounded and sanitized before bytes are read", () => {
  assertBrowserScreenshotFileBeforeRead({
    name: "capture.png",
    mime: "image/png",
    size: BROWSER_SCREENSHOT_BYTE_LIMIT,
    isFile: true,
  })
  assertThrows(
    () =>
      assertBrowserScreenshotFileBeforeRead({
        name: "capture.png",
        mime: "image/png",
        size: BROWSER_SCREENSHOT_BYTE_LIMIT + 1,
        isFile: true,
      }),
    Error,
  ...
  assertThrows(
    () => assertBrowserScreenshotFileBeforeRead({ name: "capture.png", mime: "image/png", size: 1, isFile: false }),
    Error,
    "regular",
  )
  assertThrows(
    () => assertBrowserScreenshotFileBeforeRead({ name: "../capture.png", mime: "image/png", size: 1, isFile: true }),
    Error,
    "regular",
  )
  assertThrows(
    () => assertBrowserScreenshotFileBeforeRead({ name: "capture.png", mime: "image/jpeg", size: 1, isFile: true }),
    Error,
    "regular",
  )
})
