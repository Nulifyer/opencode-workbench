import { parseHostMessage, PERMISSION_AGGREGATE_CHARACTER_LIMIT } from "@opencode-workbench/shared"
...
Deno.test("native session metadata and archive/share operations stay OpenCode-authoritative", async () => {
  ...
  globalThis.fetch = async (input, init) => {
    ...
    if (url.pathname === "/session/ses-native" && init?.method === "PATCH") {
      return new Response(JSON.stringify(session()))
    }
    if (url.pathname === "/session/ses-native/share" && init?.method === "POST") {
      return new Response(JSON.stringify(session()))
    }
    if (url.pathname === "/session/ses-native/share" && init?.method === "DELETE") {
      return new Response(JSON.stringify(session({ share: undefined })))
    }
  ...
  try {
    ...
    if (
      !native || native.parentID !== "ses-parent" || native.time.archived !== 3 ||
      native.share?.url !== "https://share.example.test/ses-native" ||
      native.summary?.files !== 1 || native.revert?.partID !== "part-one" || native.model?.variant !== "high" ||
      native.tokens?.cache.read !== 7
    ) {
    ...
    if ((await client.shareSession("ses-native")).share?.url !== "https://share.example.test/ses-native") {
      throw new Error("Native OpenCode share response was not preserved")
    }
    if ((await client.unshareSession("ses-native")).share !== undefined) {
      throw new Error("Native OpenCode unshare response was not preserved")
    }
    const archive = requests.find((request) => request.method === "PATCH")
    if (
      archive?.url.pathname !== "/session/ses-native" ||
      JSON.stringify(archive.body) !== JSON.stringify({ time: { archived: 123 } })
    ) {
      throw new Error(`Archive did not use the native OpenCode PATCH contract: ${JSON.stringify(archive)}`)
    }
    if (
      requests.filter((request) => request.url.pathname === "/session/ses-native/share").map((request) =>
        request.method
      ).join(",") !== "POST,DELETE"
    ) {
...
Deno.test("bounded experimental session search uses OpenCode and falls back compatibly", async () => {
  ...
  const session = (id: string, title: string, parentID?: string, archived?: number) => ({
    id,
    title,
    parentID,
    directory: "/work",
  ...
  globalThis.fetch = async (input) => {
    ...
    if (url.pathname === "/experimental/session") {
      ...
      return new Response(
        JSON.stringify([
          session("ses-result", "Search result", url.searchParams.has("roots") ? undefined : "ses-parent"),
        ]),
      )
    }
  ...
  try {
    ...
    const native = await client.listSessions({
      search: "result",
      roots: true,
      archived: false,
      start: 10,
    ...
    if (
      query.searchParams.get("search") !== "result" || query.searchParams.get("roots") !== "true" ||
      query.searchParams.get("archived") !== "false" ||
      query.searchParams.get("start") !== "10" || query.searchParams.get("cursor") !== "20" ||
      query.searchParams.get("limit") !== "25"
    ) throw new Error(`Experimental session query was malformed: ${query}`)
    ...
    if (fallback.map((entry) => entry.id).join(",") !== "ses-root") {
      throw new Error("Legacy session fallback did not preserve bounded root/archive/search filters")
    }
...
Deno.test("experimental session search enforces the SDK response and paging boundaries", async () => {
  ...
  try {
    ...
    if (requests.some((request) => request.pathname === "/session")) {
      throw new Error("A supported endpoint's bad request incorrectly used the legacy fallback")
    }

    response = 404
    await rejects(() => client.listSessions({ cursor: 20 }), /does not support paged session search/)
    if (requests.some((request) => request.pathname === "/session")) {
      throw new Error("Paged search incorrectly used the unpaged legacy fallback")
    }
...
Deno.test("native PTY parsing is strict, bounded, and clone-safe", () => {
  ...
  if (!pty || pty.id !== "pty-one" || pty.args[0] !== "test" || pty.exitCode !== undefined) {
    throw new Error("Valid OpenCode PTY metadata was rejected")
  }
  ...
  for (
    ...
    if (parseOpenCodePty(malformed)) {
      throw new Error(`Malformed OpenCode PTY metadata was accepted: ${JSON.stringify(malformed).slice(0, 200)}`)
    }
  }
})
...
Deno.test("native PTY and background-child methods use strict OpenCode endpoints", async () => {
  ...
  try {
    ...
    if (
      ptys.length !== 2 || ptys[1]?.exitCode !== 0 || !await client.deletePty("pty-one") ||
      !await client.backgroundChildSessions("ses-parent")
    ) {
      throw new Error("OpenCode PTY/background responses were not preserved")
    }
    if (
      requests.map((request) => `${request.method} ${request.url.pathname}`).join(",") !==
        "GET /pty,DELETE /pty/pty-one,POST /experimental/session/ses-parent/background" ||
      requests.some((request) => request.url.searchParams.get("directory") !== "/work")
    ) {
      throw new Error(
        `OpenCode PTY/background endpoints were incorrect: ${
    ...
    if (!malformedDeleteRejected || !unsafeSessionRejected) {
      throw new Error("Malformed PTY deletion or unsafe background-session input was accepted")
    }
  } finally {
    globalThis.fetch = originalFetch
  }
...
Deno.test("native PTY lists reject malformed, duplicate, and oversized server state", async () => {
  ...
  const responses: unknown[] = [
    [{ id: "duplicate", title: "One", command: "sh", args: [], cwd: "/work", status: "running", pid: 1 }, {
      id: "duplicate",
      title: "Two",
      command: "sh",
      args: [],
...
Deno.test("event stream treats instance disposal as a clean end and bounds unfinished frames", async () => {
  ...
  try {
    globalThis.fetch = () =>
      Promise.resolve(
        new Response('data: {"id":"event-1","type":"server.instance.disposed","properties":{}}\n\n', { status: 200 }),
      )
    const client = new OpenCodeClient({
      baseUrl: "http://127.0.0.1:4096",
      username: "",
    ...
    if (events[0]?.id !== "event-1" || events[0]?.type !== "server.instance.disposed") {
      throw new Error("Instance disposal did not preserve its ID or end the event stream cleanly")
    }
...
Deno.test("event stream preserves a large ordered burst without dropping frames", async () => {
  ...
  try {
    ...
    for (let index = 0; index < count; index += 1) {
      if (received[index] !== `event-${index}`) {
        throw new Error(`Event stream reordered frame ${index}: ${received[index]}`)
      }
    }
...
Deno.test("current permission events expose permission and patterns", () => {
  ...
  if (!oversized?.truncated || (oversized.pattern?.[0]?.length ?? 0) !== 20_000) {
    throw new Error("Oversized permission scope was not safely truncated")
  }
  ...
  if (
    !oversizedAggregate?.truncated || JSON.stringify(oversizedAggregate).length > PERMISSION_AGGREGATE_CHARACTER_LIMIT
  ) {
    throw new Error(
      `Permission aggregate above ${PERMISSION_AGGREGATE_CHARACTER_LIMIT} characters was not safely bounded`,
    )
  }
})
...
Deno.test("todo parsing bounds and filters server values", () => {
  ...
  if (todos.length !== 1 || todos[0]?.content !== "Do work" || todos[0]?.id !== undefined) {
    throw new Error("Todo parser rejected current server values or accepted invalid data")
  }
})
...
Deno.test("command parsing exposes bounded native autocomplete entries", () => {
  ...
  if (commands.length !== 1 || commands[0]?.name !== "review" || commands[0]?.hints?.[0] !== "scope") {
    throw new Error("Native command parser accepted invalid data or lost hints")
  }
})

Deno.test("change and question parsers bound native server data", () => {
  const changes = parseChanges([
    { file: "src/main.ts", patch: "diff", additions: 2, deletions: 1, status: "modified" },
    { file: "bad", additions: -1, deletions: 0 },
  ])
  ...
  if (parseQuestions(payload, "v2")[0]?.questions[0]?.options[0]?.label !== "Yes") {
    throw new Error("Question list was not parsed")
  }
  if (parseQuestion({ type: "question.v2.asked", properties: payload.data[0] })?.protocol !== "v2") {
    throw new Error("Question event was not parsed")
  }
  const permissions = parsePermissions({
    data: [{ id: "per_1", sessionID: "session", action: "read", resources: [".env"] }],
  }, "v2")
  if (permissions[0]?.protocol !== "v2" || permissions[0]?.pattern?.[0] !== ".env") {
    throw new Error("V2 permission list was not parsed")
  }
})
...
Deno.test("v2 request lists use location queries and native request schemas", async () => {
  ...
  try {
    ...
    await client.sendCommand("session", "goal", "status", "build", "acme/model", "high", [{
      type: "file",
      mime: "text/plain",
      url: "file:///work/plan.md",
      filename: "plan.md",
    }], "msg_command")
    ...
    if (
      questionRequest?.url.searchParams.get("location[directory]") !== "/work" ||
      permissionRequest?.url.searchParams.get("location[directory]") !== "/work"
    ) {
      throw new Error("V2 request list omitted its location directory")
    }
    if (questions[0]?.protocol !== "v2" || permissions[0]?.protocol !== "v2") {
      throw new Error("V2 requests were not preferred")
    }
    const commandBody = commandRequest?.body as {
      model?: unknown
      variant?: unknown
      messageID?: unknown
      parts?: unknown[]
    ...
    if (
      commandBody?.model !== "acme/model" || commandBody.variant !== "high" ||
      commandBody.messageID !== "msg_command" || commandBody.parts?.length !== 1
    ) throw new Error("Command model, variant, ID, or file parts were omitted")
    const replies = requests.filter((request) => request.url.pathname.endsWith("/permission/per_1/reply"))
    if ((replies[1]?.body as { message?: string })?.message !== "Use a read-only path") {
      throw new Error("Permission rejection feedback was omitted")
    }
    if (!requests.some((request) => request.url.pathname === "/mcp/docs/connect")) {
      throw new Error("MCP action used the wrong endpoint")
    }
  } finally {
    globalThis.fetch = originalFetch
  }
...
Deno.test("v2 prompt admission preserves IDs, delivery, preferences, and file parts", async () => {
  ...
  try {
    ...
    const admission = await client.sendPrompt(
      "session",
      "msg_018bcfe568001234567890abcd",
      "Review these",
      "queue",
      "build",
      "acme/model",
    ...
    const prompt = requests.find((request) => request.path.endsWith("/prompt"))?.body as {
      id?: string
      delivery?: string
      prompt?: { files?: Array<Record<string, unknown>> }
    }
    const model = requests.find((request) => request.path.endsWith("/model"))?.body as {
      model?: Record<string, unknown>
    }
    if (
      requests.map((request) => request.path).join(",") !==
        "/api/session/session/agent,/api/session/session/model,/api/session/session/prompt" ||
      prompt.id !== "msg_018bcfe568001234567890abcd" || prompt.delivery !== "queue" ||
      prompt.prompt?.files?.[0]?.uri !== "file:///work/src/main.ts?start=4&end=8" ||
    ...
    if (
      admission.admittedSeq !== 42 || admission.id !== prompt.id || admission.delivery !== "queue" ||
      admission.timeCreated !== 1_700_000_000_000
    ) {
...
Deno.test("v2 prompt admission rejects malformed and mismatched receipts", async () => {
  ...
  globalThis.fetch = (input) => {
    const url = new URL(input instanceof Request ? input.url : input.toString())
    return Promise.resolve(
      url.pathname.endsWith("/prompt") ? new Response(JSON.stringify(response)) : new Response(null, { status: 204 }),
    )
  }
  try {
    const client = new OpenCodeClient({
...
Deno.test("native manual compaction validates its durable receipt", async () => {
  ...
  try {
    ...
    if (
      JSON.stringify(body) !== JSON.stringify({ id: "msg_018bcfe568001234567890abcd" }) ||
      admission.type !== "compaction" || admission.admittedSeq !== 7
    ) {
      throw new Error(
        `Native compaction did not preserve its exact admission contract: ${JSON.stringify({ body, admission })}`,
      )
    }
  } finally {
    globalThis.fetch = originalFetch
...
Deno.test("legacy async prompts preserve custom provider model and variant", async () => {
  ...
  try {
    ...
    if (
      request?.path !== "/session/session/prompt_async" || body.agent !== "caveboss" ||
      body.model?.providerID !== "openai" ||
      body.model.modelID !== "gpt-5.6-sol" || body.variant !== "high" ||
      body.messageID !== "msg_018bcfe568001234567890abcd"
    ) {
...
Deno.test("structured verifier prompts declare their legacy transport with a stable OpenCode ID", async () => {
  ...
  try {
    ...
    const messageID = await client.sendStructuredPrompt("verifier", "verify", {
      schema: { type: "object" },
      retryCount: 2,
    })
    if (
      !/^msg_[0-9a-f]{12}[0-9A-Za-z]{14}$/.test(messageID) || body?.messageID !== messageID ||
      (body?.format as { type?: string } | undefined)?.type !== "json_schema" ||
      (body?.tools as Record<string, unknown> | undefined)?.["*"] !== false
    ) {
      throw new Error(
        `Structured prompt lost its explicit OpenCode legacy contract: ${JSON.stringify({ messageID, body })}`,
      )
    }
  } finally {
    globalThis.fetch = originalFetch
...
Deno.test("session status and interruption cover both v2 and legacy runners", async () => {
  ...
  globalThis.fetch = (input) => {
    ...
    if (url.pathname === "/session/status") {
      return Promise.resolve(new Response(JSON.stringify({ legacy: { type: "retry", attempt: 2 } })))
    }
    if (url.pathname === "/api/session/active") {
      return Promise.resolve(new Response(JSON.stringify({ data: { current: { type: "running" } } })))
    }
  ...
  try {
    ...
    if (statuses.legacy?.type !== "retry" || statuses.current?.type !== "busy") {
      throw new Error("V2 and legacy statuses were not combined")
    }
    if (
      !await client.abort("current") || !paths.includes("/api/session/current/interrupt") ||
      !paths.includes("/session/current/abort")
    ) {
...
Deno.test("v2 projected messages expose durable prompts and assistant output", async () => {
  ...
  globalThis.fetch = (input) => {
          ...
          data: [
            {
              ...
              time: { created: 1 },
              text: "hello",
              files: [{ uri: "file:///work/a.txt", mime: "text/plain", name: "a.txt" }],
            },
            {
              id: "msg_assistant",
              type: "assistant",
    ...
    return Promise.resolve(
      new Response(JSON.stringify([
        ...
        {
          info: { id: "msg_assistant", sessionID: "session", role: "assistant", time: { created: 2 } },
          parts: [
            { id: "legacy-patch", sessionID: "session", messageID: "msg_assistant", type: "patch", text: "diff" },
            {
              ...
              type: "tool",
              tool: "bash",
              state: { status: "completed", metadata: { delegation: "kept" } },
            },
          ],
        },
      ])),
  ...
  try {
    ...
    if (
      !messages.some((message) => message.info.id === "msg_old") || user?.parts.map((part) =>
          part.type
        ).join(",") !== "text,file" ||
      assistant?.parts.find((part) => part.id === "reasoning")?.text !== "Detailed provider thinking" ||
      assistant.parts.find((part) => part.id === "text")?.text !== "hi" ||
      assistant.parts.find((part) => part.id === "tool")?.state?.output !== "/work" ||
      !JSON.stringify(assistant.parts.find((part) => part.id === "tool")?.state?.metadata).includes("kept") ||
      !assistant.parts.some((part) => part.id === "legacy-patch") || assistant.info.providerID !== "acme" ||
      ...
      throw new Error(
        `V2 projected messages were not converted or merged with legacy history: ${JSON.stringify(messages)}`,
      )
    }
  } finally {
    globalThis.fetch = originalFetch
...
Deno.test("empty v2 user projections preserve legacy prompt content", async () => {
  ...
  globalThis.fetch = (input) => {
    ...
    if (url.pathname.startsWith("/api/session/")) {
      return Promise.resolve(
        new Response(
          JSON.stringify({ data: [{ id: "msg_user", type: "user", time: { created: 1 }, text: "" }], cursor: {} }),
        ),
      )
    }
    return Promise.resolve(
  ...
  try {
    ...
    const messages = await client.messages("session")
    if (messages[0]?.parts[0]?.text !== "Keep the actual prompt") {
      throw new Error("Empty v2 projection replaced the legacy prompt")
    }
  } finally {
    globalThis.fetch = originalFetch
  }
...
Deno.test("message reconciliation follows v2 and legacy pagination cursors", async () => {
  ...
  globalThis.fetch = (input) => {
    ...
    return Promise.resolve(
      new Response(JSON.stringify(data), older ? undefined : { headers: { "x-next-cursor": "legacy-older" } }),
    )
  }
  try {
    const client = new OpenCodeClient({
    ...
    const history = await client.messageHistory("session")
    if (
      history.messages.map((message) => message.info.id).join(",") !== "old,new" ||
      history.legacyMessageIDs.join(",") !== "old,new" || history.v2MessageIDs.join(",") !== "old,new" ||
      !requests.some((request) => request.includes("cursor=v2-older")) ||
      !requests.some((request) => request.includes("before=legacy-older"))
    ) throw new Error("Transcript pagination did not retrieve and order older pages")
  } finally {
    globalThis.fetch = originalFetch
  }
})
...
Deno.test("bounded message history pages advance both protocol cursors without loading the full session", async () => {
  ...
  globalThis.fetch = (input) => {
    ...
    return Promise.resolve(
      new Response(JSON.stringify([message]), older ? undefined : { headers: { "x-next-cursor": "legacy-older" } }),
    )
  }
  try {
    const client = new OpenCodeClient({
    ...
    if (
      recent.messages.map((message) => message.info.id).join(",") !== "new" || recent.cursor.legacyComplete ||
      recent.cursor.v2Complete ||
      older.messages.map((message) => message.info.id).join(",") !== "old" || !older.cursor.legacyComplete ||
      !older.cursor.v2Complete ||
      !requests.some((request) => request.includes("limit=200")) || !requests.some((request) =>
        request.includes("limit=500")
      ) ||
      !requests.some((request) => request.includes("cursor=v2-older")) || !requests.some((request) =>
        request.includes("before=legacy-older")
      )
    ) {
      throw new Error(
        `Bounded history pagination did not preserve both cursors: ${JSON.stringify({ recent, older, requests })}`,
      )
    }
  } finally {
    globalThis.fetch = originalFetch
...
Deno.test("bounded history retries a transport after a transient page failure", async () => {
  const originalFetch = globalThis.fetch
  let legacyCalls = 0
  globalThis.fetch = (input) => {
    const url = new URL(input instanceof Request ? input.url : input.toString())
    if (url.pathname.startsWith("/api/session/")) {
      return Promise.resolve(
        new Response(JSON.stringify({
          data: [{ id: "v2", type: "user", time: { created: 2 }, text: "v2" }],
          cursor: {},
        })),
      )
    }
    legacyCalls += 1
    if (legacyCalls === 1) return Promise.resolve(new Response("temporary", { status: 503 }))
    return Promise.resolve(
      new Response(
        JSON.stringify([{
          info: { id: "legacy", sessionID: "session", role: "user", time: { created: 1 } },
          parts: [],
        }]),
      ),
    )
  ...
  try {
    ...
    const first = await client.messageHistoryPage("session", undefined, 400)
    if (first.cursor.legacyComplete || !first.cursor.v2Complete) {
      throw new Error("Transient legacy failure was incorrectly treated as end-of-history")
    }
    const retried = await client.messageHistoryPage("session", first.cursor, 400)
    if (retried.messages[0]?.info.id !== "legacy" || !retried.cursor.legacyComplete || legacyCalls !== 2) {
      throw new Error("Transient legacy history was not retried")
    }
  } finally {
    globalThis.fetch = originalFetch
  }
...
Deno.test("prompt admission checks durable session history", async () => {
  ...
  globalThis.fetch = (input) => {
    ...
    return Promise.resolve(
      new Response(JSON.stringify(
        after === "0"
          ? {
            data: [{ type: "session.next.prompt.admitted", durable: { seq: 4 }, data: { messageID: "msg_target" } }],
            hasMore: false,
          }
          : { data: [], hasMore: false },
      )),
  ...
  try {
    ...
    if (
      await client.hasPromptAdmission("session", "msg_target") !== true ||
      await client.hasPromptAdmission("session", "msg_missing") !== false
    ) {
...
Deno.test("runtime and todo methods use native endpoints and catalog context limits", async () => {
  ...
  globalThis.fetch = (input) => {
    ...
    const value: unknown = url.pathname === "/config/providers"
      ...
      : url.pathname === "/agent"
      ? [{ name: "build", mode: "primary", model: { providerID: "acme", modelID: "model" }, variant: "high" }, {
        name: "hidden",
        mode: "primary",
        hidden: true,
  ...
  try {
    ...
    if (
      catalogs.models[0]?.inputLimit !== 120_000 || catalogs.models[0]?.outputLimit !== 8_000 ||
      !catalogs.models[0]?.capabilities?.input?.image
    ) throw new Error("Provider model capabilities and separate limits were not exposed")
    if (catalogs.models[0]?.variants?.join(",") !== "low,high") {
      throw new Error("Provider reasoning variants were not exposed")
    }
    if (catalogs.agents[0]?.model?.providerID !== "acme" || catalogs.agents[0]?.model?.modelID !== "model") {
      throw new Error("Agent model default was not exposed")
    }
    if (
      catalogs.agents.some((agent) => agent.name === "hidden") || catalogs.mentionAgents[0]?.name !== "research" ||
      catalogs.resources[0]?.uri !== "mcp://docs"
    ) throw new Error("Hidden agents or mention/resource catalogs were incorrect")
    if (
      catalogs.defaults.agent !== "build" || catalogs.defaults.model !== "acme/model" ||
      catalogs.defaults.variant !== "high"
    ) {
