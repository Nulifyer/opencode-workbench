...
Deno.test("controller communicates with OpenCode over authenticated HTTP and SSE", async () => {
  ...
  const server = createServer(async (request, response) => {
    ...
    if (request.method === "GET" && url.pathname === "/api/session/ses_network/message") {
      return json({ data: projectedMessages, cursor: {} })
    }
    if (request.method === "GET" && url.pathname === "/session/ses_network/message") return json([])
    if (request.method === "GET" && ["/session/ses_network/todo", "/session/ses_network/diff"].includes(url.pathname)) {
      return json([])
    }
    if (request.method === "POST" && url.pathname === "/api/session/ses_network/prompt") {
      ...
      projectedMessages = [
        { id: promptBody.id, type: "user", time: { created: 2 }, text: prompt.text },
        {
          ...
          content: [{ id: "text", type: "text", text: "Hello from OpenCode" }],
          finish: "stop",
        },
      ]
      json({
        ...
        const events = [
          {
            ...
            properties: { sessionID: "ses_network", assistantMessageID: "msg_assistant", timestamp: 3 },
          },
          {
            type: "session.next.text.started",
            properties: { sessionID: "ses_network", assistantMessageID: "msg_assistant", textID: "text" },
          },
          {
          ...
          },
          {
            ...
            properties: { sessionID: "ses_network", assistantMessageID: "msg_assistant", finish: "stop" },
          },
        ]
        for (const event of events) {
          for (const stream of eventStreams) stream.write(`data: ${JSON.stringify(event)}\n\n`)
        }
      }, 0)
      return
    }
  ...
  try {
    ...
    await eventually(() =>
      controller.chatSnapshot().session?.messages.some((message) =>
        message.info.role === "assistant" && message.parts.some((part) => part.text === "Hello from OpenCode")
      ) === true
    )
    const prompt = promptBody as
      | { id?: string; delivery?: string; resume?: boolean; prompt?: { text?: string } }
      | undefined
    if (
      sessionID !== "ses_network" || !isOpenCodeMessageID(prompt?.id) || prompt?.prompt?.text !== "Ping" ||
      prompt.delivery !== "steer" || prompt.resume !== true
    ) {
    ...
    if (controller.snapshot.sessions[sessionID]) {
      throw new Error("Successful OpenCode deletion did not update controller state")
    }
    if (unhandled.length || errors.length) {
      throw new Error(`Communication produced errors: ${[...unhandled, ...errors].join(", ")}`)
    }
    if (!requests.length || requests.some((request) => request.directory !== "/workspace")) {
      throw new Error("OpenCode requests did not retain workspace affinity")
    }
    if (
      !requests.some((request) => request.path === "/event") ||
      !requests.some((request) => request.path === "/api/session/ses_network/prompt")
    ) {
