...
Deno.test("diff identity hashes exact bytes and resolves file hunks", async () => {
  ...
  const git = {
    run: async (args: string[]) => ({
      stdout: args[0] === "ls-files" ? "" : args.includes("--numstat") ? "1\t1\tsrc/a.ts\0" : unified,
      stderr: "",
    }),
  }
...
Deno.test("turn attribution and oversized diffs fail visibly rather than silently truncating", async () => {
  const git = {
    run: async (args: string[]) => ({
      stdout: args[0] === "ls-files" ? "" : args.includes("--numstat") ? "1\t0\ta\0" : "large",
      stderr: "",
    }),
  }
...
Deno.test("native diff text reads surface Git errors and enforce byte bounds", async () => {
  ...
  await assertRejects(
    () => service.readRevisionText("/repo", "HEAD", "missing.ts"),
    Error,
    "arbitrary git show failure",
  )
...
Deno.test("evidence remains scoped to its session and repository", () => {
  ...
  assertEquals(
    service.list({ sessionID: "a", repository: "/repo" }).map((entry) => entry.label),
    ["Session A"],
  )
...
Deno.test("untracked diff capture stops at explicit byte and file limits", async () => {
  ...
  assertEquals(
    calls.filter((args) => args.includes("--no-index") && !args.includes("--numstat")).length,
    1_000,
  )
})
