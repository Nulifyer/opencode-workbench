...
Deno.test("fzf ranking is case-insensitive and rewards basename prefixes", () => {
  ...
  if (ranked[0] !== "README.md" || !ranked.includes("docs/reading-list.md")) {
    throw new Error(`Unexpected fuzzy order: ${ranked.join(", ")}`)
  }
})
...
Deno.test("fzf ranking supports sparse path queries and rejects missing characters", () => {
  ...
  if (ranked[0] !== "src/views/chat-view.ts" || fzfScore("xyz", "README.md") !== undefined) {
    throw new Error("Fuzzy subsequence matching failed")
  }
})
...
Deno.test("prepared fzf indexes preserve deterministic bounded ranking", () => {
  ...
  if (JSON.stringify(rankPreparedFzf("mod", prepared, 3)) !== JSON.stringify(expected)) {
    throw new Error("Prepared ranking changed results")
  }
...
Deno.test("workspace search includes parent directories as mention targets", () => {
  ...
  for (
    const expected of ["packages/vscode-extension", "packages/vscode-extension/src", "packages", "packages/shared"]
  ) {
  ...
  if (ranked[0] !== "packages/vscode-extension") {
    throw new Error(`Folder was not the best mention match: ${ranked.join(", ")}`)
  }
})
...
Deno.test("workspace search does not invent prefix paths for root files", () => {
  ...
  if (
    !paths.includes("README.md") || !paths.includes("src") ||
    paths.some((value) =>
