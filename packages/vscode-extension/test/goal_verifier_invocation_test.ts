...
import {
  createOpenCodeGoalVerifierInvocation,
  type OpenCodeGoalVerifierRuntime,
} from "../src/application/goal-verifier-invocation.ts"
...
Deno.test("OpenCode verifier invocation preserves schema, tool-free agent defaults, and usage metadata", async () => {
  ...
  const invoke = createOpenCodeGoalVerifierInvocation(() =>
    runtime({
...
Deno.test("OpenCode verifier invocation rejects tool use and interrupts the isolated session", async () => {
  ...
  const error = await assertRejects(
    () => invoke("Evidence only", {}, new AbortController().signal),
    GoalVerifierInvocationError,
    "forbidden tool",
  )

  assertEquals(error.metadata, {
    sessionID: "verifier-session",
    model: "provider/actual-model",
    tokens: 28,
    cost: 0.25,
...
Deno.test("an immediate structured-prompt rejection cleans up ambiguous admission", async () => {
  ...
  await assertRejects(
    () => invoke("Evidence only", {}, controller.signal),
    GoalVerifierInvocationError,
    "invocation failed",
  )
  assertEquals(aborts, 1)
