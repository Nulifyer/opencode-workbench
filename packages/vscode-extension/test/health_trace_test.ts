...
Deno.test("health center reports bounded current runtime state", () => {
  const service = new HealthService(() => ({
    ...
    capabilities: ["questions", "permissions", "questions"],
    eventStreamState: "connected",
    requestQueueDepth: 2,
    protocol: { version: 2, epoch: "epoch" },
  ...
  assertEquals(snapshot.eventStream, {
    state: "connected",
    lastEventAt: 10,
    lastReconciliationAt: 11,
    reconnectCount: 1,
  })
...
Deno.test("session traces are bounded and reject sensitive event categories", () => {
  ...
  const redacted = new TraceService().record({
    type: "request.failed",
    error:
  ...
  assertEquals(
    redacted.error,
    "Authorization: [redacted] Proxy-Authorization: [redacted] Cookie: [redacted] https://[redacted]@example.com/path",
  )
})
