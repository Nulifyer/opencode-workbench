...
function messages(sessionID: string, count: number, start = 1): MessageBundle[] {
  return Array.from({ length: count }, (_, index) => ({
    info: { id: `msg_${sessionID}_${index}`, sessionID, role: index % 2 ? "assistant" : "user", time: { created: start + index } },
    parts: index % 2 ? [{ id: `part_${index}`, sessionID, messageID: `msg_${sessionID}_${index}`, type: "tool", tool: "bash", state: { status: "running" } }] : [],
  }))
}

function delegation(sessionID: string, status: DelegationProgress["status"], revision = 1, count = 2): DelegationProgress {
  return { partID: `part_${sessionID}`, sessionID, title: `Delegate ${sessionID}`, status, revision, messages: messages(sessionID, count, 100) }
}
...
const runGroup: RunGroup = {
  ...
  runs: [{
    ...
    agent: "build",
    session: { sessionID: "ses_child", directory: "/repo/run", experience: "workbench", transport: "http-sse", runtimeEpoch: "epoch" },
    worktreeID: "wt-one",
    phase: "working",
    startedAt: 75,
  }],
...
const worktrees: WorktreeJournalEntry[] = [{
  id: "wt-one", mutationID: "mutation-one", owner: "workbench", repository: "/repo", repositoryID: "repo",
  path: "/repo/run", branch: "workbench/run", baseRef: "main", phase: "session-ready", sessionID: "ses_child", createdAt: 40, updatedAt: 80,
}, {
  id: "wt-orphan", mutationID: "mutation-two", owner: "workbench", repository: "/repo", repositoryID: "repo",
  path: "/repo/orphan", branch: "workbench/orphan", baseRef: "main", phase: "cleanup-pending", createdAt: 30, updatedAt: 90,
}]
...
Deno.test("jobs deduplicate run child sessions and referenced worktrees into the authoritative run", () => {
  ...
  assertEquals(jobs[1]!.recentActivity?.at(-1), { messageID: "msg_ses_child_1", role: "assistant", tool: "bash", toolStatus: "running" })
})
...
Deno.test("pending runs do not expose the repository as a created worktree", () => {
  const pendingGroup: RunGroup = {
    ...runGroup,
    runs: [{ ...runGroup.runs[0]!, phase: "preparing", session: { ...runGroup.runs[0]!.session, sessionID: "pending", directory: runGroup.repository } }],
  }
