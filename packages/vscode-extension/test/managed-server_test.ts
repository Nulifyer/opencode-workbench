...
Deno.test("injects packaged plugin into JSONC without replacing existing configuration", () => {
  ...
  assertEquals(JSON.parse(managedConfigContent('{"plugin":["file:///workbench.js"]}', "file:///workbench.js")).plugin, [
    "file:///workbench.js",
  ])
  assertEquals(
    JSON.parse(managedConfigContent('{"plugin":[["file:///configured.js",{"flag":true}]]}', "file:///workbench.js"))
      .plugin,
    [["file:///configured.js", { flag: true }], "file:///workbench.js"],
  )
  let rejected = false
  try {
...
Deno.test("runs Windows command shims through the command interpreter", () => {
  assertEquals(
    ...
    ),
    {
      ...
      env: {
        OPENCODE_WORKBENCH_CMD_ARG_0: "C:\\Program Files\\OpenCode\\opencode.cmd",
        OPENCODE_WORKBENCH_CMD_ARG_1: "--version",
      },
    },
  )
  const percent = executableInvocation("C:\\100% Real\\opencode.cmd", ["%PATH%"], "win32", "cmd.exe")
  assertEquals(percent.env, {
    OPENCODE_WORKBENCH_CMD_ARG_0: "C:\\100% Real\\opencode.cmd",
    OPENCODE_WORKBENCH_CMD_ARG_1: "%PATH%",
  })
...
`,
    ...
    const manager = new ManagedOpenCodeServer({
      ...
      environment: { ...process.env, OPENCODE_WORKBENCH_TEST_ENV: "isolated" },
    })
    try {
...
`,
    ...
    try {
      ...
      if (
        !(deadline && Number(deadline) > 0 && Number(deadline) <= 25) &&
        message !== "Managed OpenCode startup deadline expired" &&
        message !== "Managed OpenCode server did not become ready within 0.025 seconds"
      ) {
