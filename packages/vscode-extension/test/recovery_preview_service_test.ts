...
import { RecoveryPreviewGuard, RecoveryPreviewService, type RecoveryPreviewInput } from "../src/application/recovery-preview-service.ts"
...
Deno.test("recovery preview reports the exact transcript tail and conservative current file summaries", () => {
  const messages = [message("msg_one", "user", "First"), message("msg_two", "assistant"), message("msg_three", "user", "Try again"), message("msg_four", "assistant")]
  const changes: FileChange[] = [{ file: "src/main.ts", patch: "not exposed", additions: 4, deletions: 2, status: "modified" }]
  const preview = new RecoveryPreviewService().preview({
    sessionID: "ses_one", status: { type: "idle" }, messages, changes, messageID: "msg_three", revertMessageID: "msg_one",
  })
...
Deno.test("recovery preview defaults to the latest user message without mutating the transcript", () => {
  const messages = [message("msg_one", "user", "First"), message("msg_two", "assistant"), message("msg_three", "user", "Latest")]
  const preview = new RecoveryPreviewService().preview({ sessionID: "ses_one", status: { type: "idle" }, messages, changes: [] })
...
Deno.test("native redo preview is dedicated to OpenCode's current revert marker", () => {
  const messages = [message("msg_one", "user", "Original request"), message("msg_two", "assistant", "Original response")]
  const changes: FileChange[] = [{ file: "src/main.ts", patch: "@@ current", additions: 2, deletions: 1, status: "modified" }]
  ...
  assertThrows(() => service.preview({ sessionID: "ses_one", status: { type: "idle" }, messages, changes, intent: "redo" }), Error, "native OpenCode revert marker")
  assertThrows(() => service.preview({ sessionID: "ses_one", status: { type: "idle" }, messages, changes, intent: "redo", messageID: "msg_two", revertMessageID: "msg_one" }), Error, "must match")
})
...
Deno.test("recovery preview validates idle state, user boundaries, and exact projection bounds", () => {
  ...
  assertThrows(() => service.preview({ sessionID: "ses_one", status: { type: "busy" }, messages, changes: [] }), Error, "idle")
  assertThrows(() => service.preview({ sessionID: "ses_one", status: { type: "idle" }, messages, changes: [], messageID: "msg_two" }), Error, "user message")
  assertThrows(() => service.preview({ sessionID: "ses_one", status: { type: "idle" }, messages: [messages[0]!, messages[0]!], changes: [] }), Error, "unique")
  assertThrows(() => service.preview({
    sessionID: "ses_one",
    status: { type: "idle" },
    messages: Array.from({ length: 5_001 }, (_, index) => message(`msg_${index}`, index === 0 ? "user" : "assistant")),
    changes: [],
    messageID: "msg_0",
  }), Error, "exact-preview limit")
})
...
Deno.test("recovery confirmation rejects transcript, change, and native revert drift", () => {
  ...
  const drifts: Array<(input: RecoveryPreviewInput) => RecoveryPreviewInput> = [
    (input) => ({ ...input, messages: [message("msg_one", "user", "Changed prompt"), message("msg_two", "assistant", "Done")] }),
    (input) => ({ ...input, changes: [{ file: "src/main.ts", patch: "@@ after", additions: 1, deletions: 0, status: "modified" }] }),
  ...
  for (const drift of drifts) {
    ...
    assertThrows(() => guard.consume(surface, () => ({ input: changed, preview: service.preview(changed) })), Error, "changed")
    assertThrows(() => guard.consume(surface, () => ({ input: original, preview: service.preview(original) })), Error, "missing or stale")
  }
})
...
Deno.test("failed recovery revalidation consumes a preview after status drift", () => {
  ...
  const guard = new RecoveryPreviewGuard<object>()
  const surface = {}
  const input: RecoveryPreviewInput = { sessionID: "ses_one", status: { type: "idle" }, messages: [message("msg_one", "user", "Do it")], changes: [] }
  guard.remember(surface, { input, preview: service.preview(input) })
  assertThrows(() => guard.consume(surface, (preview) => {
    const changed: RecoveryPreviewInput = { ...input, status: { type: "busy" } }
    return { input: changed, preview: service.preview(changed) }
  }), Error, "idle")
  assertThrows(() => guard.consume(surface, () => ({ input, preview: service.preview(input) })), Error, "missing or stale")
})
