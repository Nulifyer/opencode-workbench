...
Deno.test("walkthrough rejects invented anchors and oversized input explicitly", async () => {
  ...
  await assertRejects(
    () =>
      invalid.generate(capture, "model", async () =>
        JSON.stringify({
          coverage: "complete",
          stops: [{
            title: "Bad",
            explanation: "Bad",
            importance: "normal",
            anchors: [{ file: "invented.ts", side: "modified", startLine: 1, endLine: 1 }],
          }],
        })),
    Error,
    "unknown file",
...
Deno.test("walkthrough insertion retains only the newest protocol-safe 100 documents", async () => {
  ...
  const service = new WalkthroughService(initial, (documents) => {
    persisted = documents
  })
  const generated = await service.generate(capture, "provider/new-model", async () =>
    JSON.stringify({
      coverage: "complete",
      stops: [{
        title: "Change",
