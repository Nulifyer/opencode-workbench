...
Deno.test("production webview client negotiates, correlates, and replays sequence gaps", async () => {
  ...
  assert(hello.protocolRange?.minimum === 2 && hello.protocolRange.maximum === 2 && hello.client?.surfaceID === "surface", "Client did not initiate v2 negotiation");
  ...
  assert(action.kind === "request" && action.type === "workbench.dispatch" && action.id && action.mutationID && action.payload?.value === 1, "Client action was not wrapped in a correlated mutation request");
  ...
  assert(resync.type === "protocol.resync" && resync.payload?.lastSeenSequence === 0, "Sequence gap did not request deterministic recovery");
  ...
  assert(received.slice(-3).map((message) => message.value).join(",") === "1,2,3", "Replay did not restore missing events and drain an interleaved live event");
  ...
  target.dispatch({
    ...
    type: "workbench.snapshot",
    revision: 3,
    payload: { snapshot: { epoch: "epoch", sequence: 3, revision: 3, state: [{ type: "stale-snapshot", value: 100 }] } },
  });
...
Deno.test("new webview client falls back when a legacy host answers hello", () => {
  ...
  assert(client.protocol === 1 && received[0]?.type === "error", "Legacy host response did not select v1 compatibility mode");
...
Deno.test("disposing a v2 webview requests cancellation for in-flight host work", () => {
  ...
  assert(pending.type === "workbench.dispatch" && cancellation.type === "protocol.cancel" && cancellation.payload?.requestID === pending.id, "Webview disposal did not cancel its in-flight request");
});
