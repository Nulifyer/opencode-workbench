import { capabilitiesForRuntime, type ProtocolLimits, type Request } from "@opencode-workbench/shared"
...
Deno.test("production webview host negotiates, correlates, recovers, and keeps v1 compatibility", async () => {
  ...
  const initial = posted[1] as {
    kind?: string
    type?: string
    sequence?: number
    payload?: { snapshot?: { state?: Action[] } }
  }
  assert(ready.protocol === 2 && ready.capabilities?.["session.create"], "Negotiated capabilities were not published")
  assert(
    initial.kind === "event" && initial.type === "workbench.snapshot" && initial.sequence === 0,
    "Initial v2 snapshot was not published at the initial cursor",
  )
  ...
  assert(
    observations.some((entry) =>
      entry.type === "protocol.request.completed" && entry.requestID === "one" && entry.transition === "dispatch:ok"
    ),
    "Completed request trace lost its safe correlation fields",
  )

  posted.length = 0
  state[0] = { type: "snapshot", value: 1 }
  await host.publishTo("surface", { type: "patch", value: 1 })
  const firstEvent = posted[0] as {
    sequence?: number
    revision?: number
    payload?: { baseRevision?: number; nextRevision?: number }
  }
  assert(
    firstEvent.sequence === 1 && firstEvent.revision === 1 && firstEvent.payload?.baseRevision === 0 &&
      firstEvent.payload.nextRevision === 1,
    "Live patch did not carry exact event/revision positions",
  )
  assert(
    observations.some((entry) => entry.type === "protocol.event.published" && entry.revision === 1),
    "Published event trace lost its revision",
  )
  ...
  assert(
    overflowRecovery.type === "workbench.snapshot" && overflowRecovery.payload?.snapshot?.state?.[0]?.value === 4,
    "Hidden queue overflow did not recover with current state",
  )
  ...
  assert(
    rotated.epoch !== previousEpoch && rotated.sequence === 0,
    "Runtime generation did not replace the webview epoch and cursor",
  )
...
Deno.test("negotiated capability checks reject unavailable companion actions", async () => {
  ...
  assert(
    !response.ok && response.error?.code === "CAPABILITY_UNAVAILABLE",
    "Unavailable negotiated capability was admitted",
  )
})
...
Deno.test("private composer payload is a revisioned follow-up rather than snapshot state", async () => {
  ...
  assert(
    snapshot.payload?.snapshot?.state?.some((message) => message.type === "composer-private") === false,
    "Private composer bytes leaked into snapshot state",
  )
  assert(
    followup.sequence === 1 && followup.revision === 1 && followup.payload?.message?.type === "composer-private",
    "Private composer state was not rehydrated as a revisioned follow-up",
  )
})
...
Deno.test("protocol trace categorizes resync, cancellation, and cancelled dispatch settlement", async () => {
  ...
  await host.receive("trace", {
    protocolRange: { minimum: 2, maximum: 2 },
    client: { surfaceID: "trace", extensionVersion: "test" },
  })
  ...
  await host.receive("trace", {
    ...
    payload: { epoch: host.currentEpoch, lastSeenSequence: 0 },
  })
  await host.receive("trace", {
    protocol: 2,
    kind: "request",
    id: "resync-stale",
    type: "protocol.resync",
    payload: { epoch: "stale-epoch", lastSeenSequence: 0 },
  })

  assert(
    observations.some((entry) => entry.requestID === "cancel" && entry.transition === "cancel:ok"),
    "Cancel lifecycle was not traced",
  )
  assert(
    observations.some((entry) =>
      entry.type === "protocol.request.cancelled" && entry.requestID === "slow" && entry.transition === "cancelled"
    ),
    "Cancel trace lost the target request outcome",
  )
  assert(
    observations.some((entry) => entry.requestID === "slow" && entry.transition === "dispatch:CANCELLED"),
    "Cancelled dispatch settlement was not traced",
  )
  assert(
    observations.some((entry) => entry.requestID === "resync" && entry.transition === "resync:ok"),
    "Resync lifecycle was not traced",
  )
  assert(
    observations.some((entry) =>
      entry.type === "protocol.resync.recovered" && entry.requestID === "resync" && entry.transition === "replay"
    ),
    "Resync trace did not distinguish replay recovery",
  )
  assert(
    observations.some((entry) =>
      entry.type === "protocol.resync.recovered" && entry.requestID === "resync-stale" &&
      entry.transition === "snapshot" && entry.revision === 0
    ),
    "Stale-epoch resync trace did not identify authoritative snapshot recovery",
  )
})
