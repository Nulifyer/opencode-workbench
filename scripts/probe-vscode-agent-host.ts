...
function optionNames(help: string): string[] {
  return [...help.matchAll(/^\s+--([a-z][a-z0-9-]*)\b/gim)].map((match) =>
    match[1]
  ).sort();
}
...
export async function probeAgentHost(
  ...
  const [versionOutput, hostHelp, product, extensionPackage] = await Promise
    .all([
      ...
      options.extensionPackageJson
        ? readJson(options.extensionPackageJson)
        : Promise.resolve({} as JsonObject),
    ]);
  const [version = "unknown", commit = "unknown", architecture = "unknown"] =
    versionOutput.trim().split(/\r?\n/);
  ...
  const chatSessionsProvider = proposals.find((entry) =>
    entry.proposal === "chatSessionsProvider"
  );
  const publicMarketplaceRegistrationProven =
  ...
  return {
    ...
    vscode: {
      channel: options.codeExecutable.toLowerCase().includes("insiders")
        ? "insiders"
        : "stable",
      version,
      commit,
    ...
    extension: {
      id: extensionID,
      version: typeof extensionPackage.version === "string"
    ...
    conclusions: {
      ...
      standaloneHostAcceptsThirdPartyAdapter:
        customAdapterRegistrationOptionNames.length > 0,
      nativePickerRegistration: publicMarketplaceRegistrationProven
        ? "public"
        : "blocked-proposed-api-allowlist",
      productionDependencyAllowed: false,
    },
